...
class PolynomialTermNode {
  ...
  constructor(node, onlyImplicitMultiplication=false) {
    if (Node.Type.isOperator(node)) {
      if (node.op === '^') {
        ...
        if (!Node.Type.isSymbol(symbolNode)) {
      ...
      // this means there's a fraction coefficient
        ...
        if (!Node.Type.isConstant(denominatorNode)) {
          throw Error('denominator must be constant node, instead of ' +
            denominatorNode);
        }
        const numeratorNode = new PolynomialTermNode(
        ...
        this.coeff = Node.Creator.operator(
  ...
  getCoeffNode(defaultOne=false) {
    if (!this.coeff && defaultOne) {
      return Node.Creator.constant(1);
    }
    else {
      return this.coeff;
    }
  ...
  hasFractionCoeff() {
    ...
    return this.coeff && Node.Type.isOperator(this.coeff);
  }
...
// what does not.
PolynomialTermNode.isPolynomialTerm = function(
    node, onlyImplicitMultiplication=false) {
  try {
    // will throw error if node isn't poly term
    new PolynomialTermNode(node, onlyImplicitMultiplication);
    return true;
  }
  catch (err) {
    return false;
...
function negativeCoefficient(node) {
  if (Node.Type.isConstant(node)) {
    node = Node.Creator.constant(0 - parseFloat(node.value));
  }
  else {
    const numeratorValue = 0 - parseFloat(node.args[0].value);
    node.args[0] = Node.Creator.constant(numeratorValue);
  }
  return node;
}
