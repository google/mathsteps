...
PolynomialTermOperations.rearrangeCoefficient = function(node) {
  ...
  if (NodeType.isConstantOrConstantFraction(newNode.args[0])) {
    constNode = newNode.args[0];
    polyNode = new PolynomialTermNode(newNode.args[1]);
  }
  else {
    constNode = newNode.args[1];
    polyNode = new PolynomialTermNode(newNode.args[0]);
  }
