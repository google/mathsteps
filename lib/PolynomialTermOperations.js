...
function addlikeTermPolynomialNodes(node) {
  ...
  nodeStatus =  NodeStatus.nodeChanged(                                         // TODO - change groups better
  ...
  return NodeStatus.nodeChanged(
    MathChangeTypes.ADD_POLYNOMIAL_TERMS, node, newNode, true, subSteps);
}
...
function addPositiveOneCoefficient(node) {                                      // TODO - change groups better
  const oldNode = node;
  ...
  if (change) {
    return NodeStatus.nodeChanged(
        MathChangeTypes.ADD_COEFFICIENT_OF_ONE, oldNode, newNode);
  }
  else {
    return NodeStatus.noChange(newNode);
  }
...
function addNegativeOneCoefficient(node) {                                      // TODO - change groups better
  const oldNode = node;
  const newNode = clone(node, false);
  let change = false;
  ...
  if (change) {
      return NodeStatus.nodeChanged(
          MathChangeTypes.UNARY_MINUS_TO_NEG_ONE, oldNode, newNode);
    }
    else {
      return NodeStatus.noChange(newNode);
    }
...
function evaluateCoefficientSum(node) {
  ...
  const coefficientSum = newNode.args[0];
  coefficientSum.changeGroup = 1;
  let newCoefficient = evaluateConstantSum(coefficientSum).newNode;             // TODO steps
  newCoefficient.changeGroup = 1;
  newNode.args[0] = newCoefficient;
  return NodeStatus.nodeChanged(
    MathChangeTypes.ADD_POLYNOMIAL_TERMS, oldNode, newNode, false);
}
