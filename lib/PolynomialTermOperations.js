...
PolynomialTermOperations.multiplyLikeTermPolynomialNodes = function(node) {
  ...
  // STEP 1: change anything with no exponent to have exponent 1
  ...
  */
  ...
  // e.g. x^1 * x^3 -> x^(1+3)
  let status = collectExponents(clone(node, false));
  subSteps.push(status);
  // clone and reset
  oldNode = NodeStatus.resetChangeGroups(status.newNode);

  // STEP 3: add exponents together.
  // NOTE: This might not be a change if the exponents aren't all constants,
