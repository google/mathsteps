...
Symbols.getLastSymbolTerm = function(node, symbolName) {
  ...
  if (isSymbolTerm(node, symbolName)) {
    // Node itself is a polyomial term with symbolName.
    rv = node

  } else if (Node.Type.isOperator(node, '+')) {
    // If it's a sum of terms, look through the operands for a term
    // with `symbolName`
    // a+b+c+d+... - try terms one-by-one beginning from the last one.
    for (let idx = node.args.length - 1; idx >= 0 ; idx--) {
      // Search for symbol within current factor.
      rv = Symbols.getLastSymbolTerm(node.args[idx], symbolName)

      if (rv) {
        // Term constains symbol, we're searched for.
        // Don't go on anymore.
        break
      }
    }
    ...
    rv = Symbols.getLastSymbolTerm(node.content, symbolName)

  } else if (Node.Type.isOperator(node, '*')) {
    // a*b*c*... - try all factors one-by-one.

    for (let idx = 0; idx < node.args.length; idx++) {
      // Search for symbol within current factor.
      if (Symbols.getLastSymbolTerm(node.args[idx], symbolName)) {
        // At least one factor contains symbol, we're searched for.
        // a * b * ... * x * ... * d * e * ...
        // Treat whole node as one term "with x".
        rv = node
