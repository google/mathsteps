...
TreeSearch.preOrder = function(simplificationFunction) {
  return function (node, expressionCtx) {
    return search(simplificationFunction, node, true, expressionCtx)
  }
}
...
TreeSearch.postOrder = function(simplificationFunction) {
  return function (node, expressionCtx) {
    return search(simplificationFunction, node, false, expressionCtx)
  }
}

// A helper function for performing a tree search with a function
function search(simplificationFunction, node, preOrder, expressionCtx) {
  let status

  if (preOrder) {
    status = simplificationFunction(node, expressionCtx)
  ...
  if (Node.Type.isConstant(node) || Node.Type.isSymbol(node)) {
    ...
    status = search(simplificationFunction, node.args[0], preOrder, expressionCtx)
    if (status.hasChanged()) {
    ...
    for (let i = 0; i < node.args.length; i++) {
      ...
      const childNodeStatus = search(simplificationFunction, child, preOrder, expressionCtx)
      if (childNodeStatus.hasChanged()) {
        return  Node.Status.childChanged(node, childNodeStatus, i)
      }
    }
  } else if (Node.Type.isParenthesis(node)) {
    status = search(simplificationFunction, node.content, preOrder, expressionCtx)
    if (status.hasChanged()) {
      return Node.Status.childChanged(node, status)
    }
  } else {
