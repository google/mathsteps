...
function canAddLikeTermWithCoefficientNodes(node, termWithCoefficientSubclass) {
  if (!Node.Type.isOperator(node) || node.op !== '+') {
    return false;
  }
  const args = node.args;
  if (!args.every(n => Node.TermWithCoefficient.isTermWithCoefficient(
    n, termWithCoefficientSubclass.baseNodeFunc))) {
    return false;
  }
  if (args.length === 1) {
    return false;
  ...
  const termList = args.map(n => new termWithCoefficientSubclass(n));
