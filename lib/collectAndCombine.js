...
function collectAndCombineLikeTerms(node) {
  if (node.op === '+') {
    return collectAndCombineAddition(node);
  }
  else if (node.op === '*') {
    return collectAndCombineMultiplication(node);
  }
  else {
    return NodeStatus.noChange(node);
  }
...
function collectAndCombineAddition(node) {
  ...
  status = PolynomialTermOperations.addlikeTermPolynomialNodes(oldNode);
...
function collectAndCombineMultiplication(node) {
  ...
  // STEP 3: combine the polynomial terms (there's always at least 2)
