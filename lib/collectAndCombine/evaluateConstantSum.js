...
const addConstantFractions = require('../simplifyFractions/addConstantFractions');
const addConstantAndFraction = require('../simplifyFractions/addConstantAndFraction');
...
function evaluateConstantSum(node) {
  ...
  if (NodeType.isParenthesis(constants)) {
    ...
    const evaluateStatus = evaluateArithmeticSearch(constantList);
...
function groupConstantsAndFractions(node) {
  ...
  return NodeStatus.nodeChanged(
    MathChangeTypes.COLLECT_LIKE_TERMS, node, newNode);
}
