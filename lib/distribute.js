...
function distributeAndSimplifyOperationNode(node) {
  ...
  const oldNode = clone(node, false); // the "before step 1" node
  for (let i = 0; i+1 < oldNode.args.length; i++) {
    ...
    if (newNode.args.length > 2) {
      ...
      const childSimplifyStatus = simplifyParens(clone(newNode.args[i], false));
    ...
    // Now we can just simplify it.
      ...
      simplifyStatus = simplifyParens(clone(newNode, false));
    }
    ...
    if (!simplifyStatus || !simplifyStatus.hasChanged()) {
      ...
      const finalNode = NodeStatus.resetChangeGroups(simplifyStatus.newNode);
      return NodeStatus.nodeChanged(
        MathChangeTypes.DISTRIBUTE, originalNode, finalNode, false, substeps); // change groups will be wrong here for case 1
    }
  }
  return NodeStatus.noChange(node);
}
