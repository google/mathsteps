...
function distributeOperation(node) {
  ...
  for (let i = 0; i+1 < oldNode.args.length; i++) {
    ...
    if (isParenthesisOfAddition(firstArg) ||
      ...
      secondArg.changeGroup = 1; // TODO we might want to consider making this a different colour
...
function distributeTwoNodes(firstNode, secondNode) {
  ...
  if (firstArgs.length > 1 && secondArgs.length > 1) {
    firstArgs.forEach(leftArg => {
      let arg = NodeCreator.operator('*', [clone(leftArg, false), clone(secondNode, false)]);
      arg.changeGroup = 1;
      newArgs.push(arg);
    });
  }
    ...
    firstArgs.forEach(leftArg => {
      secondArgs.forEach(rightArg => {
        let arg = NodeCreator.operator('*', [clone(leftArg, false), clone(rightArg, false)]);
        arg.changeGroup = 1;
        newArgs.push(arg);
      });
    });
