...
// changed.
var Status = (function () {
    function Status(changeType, oldEquation, newEquation, substeps) {
    ...
    Status.prototype.hasChanged = function () {
...
// hasn't been changed.
Status.noChange = function (equation) { return new Status(ChangeTypes.NO_CHANGE, null, equation); };
Status.addLeftStep = function (equation, leftStep) {
    var substeps = [];
    leftStep.substeps.forEach(function (substep) {
        substeps.push(Status.addLeftStep(equation, substep));
    });
    var oldEquation = null;
    if (leftStep.oldNode) {
        oldEquation = equation.clone();
        oldEquation.leftNode = leftStep.oldNode;
    }
    var newEquation = equation.clone();
    newEquation.leftNode = leftStep.newNode;
    return new Status(leftStep.changeType, oldEquation, newEquation, substeps);
};
Status.addRightStep = function (equation, rightStep) {
    var substeps = [];
    rightStep.substeps.forEach(function (substep) {
    ...
    return new Status(rightStep.changeType, oldEquation, newEquation, substeps);
};
Status.resetChangeGroups = function (equation) {
    var leftNode = mathNode.Status.resetChangeGroups(equation.leftNode);
    var rightNode = mathNode.Status.resetChangeGroups(equation.rightNode);
