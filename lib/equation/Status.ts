...
// hasn't been changed.
Status.noChange = equation => new Status(ChangeTypes.NO_CHANGE, null, equation);

Status.addLeftStep = (equation, leftStep) => {
    const substeps = [];
...
Status.addRightStep = (equation, rightStep) => {
    const substeps = [];
...
Status.resetChangeGroups = equation => {
