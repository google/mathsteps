...
function evaluateArithmeticDFS(node) {
  ...
  if (NodeType.isParenthesis(node)) {
    ...
    if (innerNodeStatus.hasChanged()) {
      let newNode = innerNodeStatus.newNode;
      if (NodeType.isConstantOrConstantFraction(newNode)) {
        // get rid of the parens by doing a node changed insead of child changed
        const oldNode = node;
        return NodeStatus.nodeChanged(
          innerNodeStatus.changeType, oldNode, newNode);
      }
    }
  }
