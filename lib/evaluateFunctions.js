...
// If the root is a positive constant, it factors the multiplicands, groups them
// and converts the multiplications into exponents.
// Distributes the nthRoot into the children nodes, evaluates those nthRoots
// and combines them
...
function combineRoots(node) {
  ...
  for (let i = 0; i < newNode.args.length; i++) {
    const child = newNode.args[i];
    if (NodeType.isFunction(child, 'nthRoot') && getRootNode(child).equals(rootNode)) {
