...
ConstantFraction.simplifyFraction = function(fraction) {
  ...
  const numeratorValue = parseInt(fraction.args[0].value);
  const denominatorValue = parseInt(fraction.args[1].value);
...
ConstantFraction.addConstantFractions = function(node) {
  ...
  if (!node.args.every(NodeType.isIntegerFraction)) {
    return new NodeStatus(node);
  }
...
// Returns a NodeStatus object.
ConstantFraction.multiplyConstantsAndFractions = function(node) {
  if (!NodeType.isOperator(node) || node.op !== '*') {
    return new NodeStatus(node);
  }
  const argsAreConstantsAndFractions = node.args.every(arg => {
    return NodeType.isIntegerFraction(arg) || NodeType.isConstant(arg);
  });
  const atLeastOneFraction = node.args.some(NodeType.isIntegerFraction);
  ...
  node.args.forEach(operand => {
    ...
    else if (NodeType.isIntegerFraction(operand)) {
