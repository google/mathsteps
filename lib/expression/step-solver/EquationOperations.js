...
function performTermOperationOnEquation(equation, op, term, changeType) {
  const leftTerm = term.clone();
  const rightTerm = term.clone();
  const leftNode = NodeCreator.operator(op, [equation.leftNode, leftTerm]);
  const rightNode = NodeCreator.operator(op, [equation.rightNode, rightTerm]);
