...
function performTermOperationOnEquation(equation, op, term, changeType) {
  let leftNode = performTermOperationOnExpression(equation.leftNode, op, term);
  let rightNode = performTermOperationOnExpression(equation.rightNode, op, term);

  let comparator = equation.comparator;
  if (Negative.isNegative(term) && (op === '*' || op === '/')) {
    comparator = COMPARATOR_TO_INVERSE[equation.comparator];
  }
