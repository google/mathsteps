...
Fraction.simplifyFraction = function(fraction) {
  ...
  let numerator = oldFraction.args[0];
  let denominator = oldFraction.args[1];
...
Fraction.multiplyByInverse = function(node) {
  ...
  return NodeStatus.nodeChanged(
    MathChangeTypes.MULTIPLY_BY_INVERSE, oldNode, newNode);
}
...
function multiplyFractions(node) {
  ...
  const oldNode = node;
  oldNode.changeBlock = 1;
  return NodeStatus.nodeChanged(MathChangeTypes.MULTIPLY_FRACTIONS, oldNode, newNode);
}
...
function breakUpNumerator(node) {
  ...
  oldNode.changeBlock = 1;
  return NodeStatus.nodeChanged(
    MathChangeTypes.BREAK_UP_FRACTION, oldNode, newNode);
}
