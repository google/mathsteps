...
Fraction.simplifyFraction = function(fraction) {
  ...
  if (Negative.isNegative(denominator)) {
    ...
    fraction.changeBlock = 1;
    newFraction.changeBlock = 1;
    return new NodeStatus(newFraction, MathChangeTypes.SIMPLIFY_FRACTION, fraction);
  }
  else {
    return new NodeStatus(fraction);
  }
...
Fraction.multiplyByInverse = function(node) {
  ...
  return new NodeStatus(newNode, MathChangeTypes.MULTIPLY_BY_INVERSE, node);
}
...
function functionOperationDFS(node, func) {
  if (NodeType.isConstant(node) || NodeType.isSymbol(node)) {
    ...
    return new NodeStatus(node, status.changeType, null, status);
  }
    ...
    for (let i = 0; i < node.args.length; i++) {
      ...
      if (childNodeStatus.hasChanged) {
        ...
        return new NodeStatus(
          node, childNodeStatus.changeType, null, childNodeStatus, i);
      }
    }
    ...
    return new NodeStatus(
      node, contentNodeStatus.changeType, null, contentNodeStatus);
  }
  else {
...
function multiplyFractions(node) {
  ...
  newNode.changeBlock = 1;
  node.changeBlock = 1;
  return new NodeStatus(newNode, MathChangeTypes.MULTIPLY_FRACTIONS, node);
}
...
function breakUpNumerator(node) {
  ...
  node.changeBlock = 1;
  return new NodeStatus(fractionSum, MathChangeTypes.BREAK_UP_FRACTION, node);
}
