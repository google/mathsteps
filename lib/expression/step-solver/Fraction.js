...
Fraction.simplifyFraction = function(fraction) {
  ...
  if (Negative.isNegative(denominator)) {
    ...
    fraction.changeBlock = 0;
    newFraction.changeBlock = 0;
...
Fraction.multiplyByInverse = function(node) {
  ...
  return new NodeStatus(newNode, MathChangeTypes.MULTIPLY_BY_INVERSE);
}
...
Fraction.breakUpNumeratorDFS = function(node) {
  if (NodeType.isConstant(node) || NodeType.isSymbol(node)) {
    ...
    return new NodeStatus(node, status.changeType);
  }
    ...
    for (let i = 0; i < node.args.length; i++) {
      ...
      if (childNodeStatus.hasChanged) {
        ...
        return new NodeStatus(node, childNodeStatus.changeType);
      }
    }
    ...
    return new NodeStatus(
      node, contentNodeStatus.changeType);
  }
  else {
...
function multiplyFractions(node) {
  ...
  return new NodeStatus(newNode, MathChangeTypes.MULTIPLY_FRACTIONS);
}
...
function breakUpNumerator(node) {
  ...
  return new NodeStatus(fractionSum, MathChangeTypes.BREAK_UP_FRACTION);
}
