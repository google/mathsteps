...
class PolynomialTermNode {
  constructor(node, onlyImplicit=false) {
    if (NodeType.isOperator(node)) {
      ...
      // it's '*' ie it has a coefficient
        ...
        const nonCoefficientTerm = new PolynomialTermNode(node.args[1], onlyImplicit);
        if (nonCoefficientTerm.hasCoeff()) {
          throw Error('Cannot have two coefficients ' + coeffNode.toString() +
            ' and ' + nonCoefficientTerm.getCoeffNode().toString());
        }
      ...
      // this means there's a fraction coefficient
        ...
        const numeratorNode = new PolynomialTermNode(node.args[0], onlyImplicit);
      ...
      const polyNode = new PolynomialTermNode(node.args[0], onlyImplicit);
...
PolynomialTermNode.isPolynomialTerm = function(
    node, debug=false, onlyImplicit=false) {
  try {
    new PolynomialTermNode(node, onlyImplicit); // will throw error if node isn't poly term
    return true;
  }
  catch (err) {
    if (debug) {
