...
function addStep(steps, nodeStatus, debug) {
  ...
  if (debug) {
    ...
    if (nodeStatus.beforeChangeNode) {
      const before = nodeStatus.beforeChangeNode;
      const after = nodeStatus.node;
      before.filter(node => node.changeBlock !== undefined).forEach(change => {
        console.log('before change: ' + change);
      })
      after.filter(node => node.changeBlock !== undefined).forEach(change => {
        console.log('after change: ' + change);
      })
    }
  }
  return steps;
...
function plusMinusToMinus(node) {
  ...
  if (exprString.match(/\+ \-/g)) {
    return new NodeStatus(node, MathChangeTypes.RESOLVE_ADD_UNARY_MINUS);
  }
  else {
    return new NodeStatus(node);
  }
