...
function stepThrough(node, debug=false, firstTime=true) {
  ...
  if (steps.length > 0) {
    ...
    steps[steps.length - 1].latex = print(nodeStatus.newNode, true);
...
function addStep(steps, nodeStatus, debug) {
  ...
  let asciimath = print(nodeStatus.newNode);
  let latex = print(nodeStatus.newNode, true);
...
function plusMinusToMinus(node) {
  ...
  node = removeUnnecessaryParens(node, true);
  // Then look for + -
  const exprString = print(node, false, true);
