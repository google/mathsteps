...
function simplifyOperationsAfterRecursion(node) {
  const simplificationFunctions = [
    ...
    ConstantFraction.multiplyConstantsAndFractions,
...
function absoluteValue(node) {
  ...
  if (NodeType.isConstant(argument)) {
    const newNode = NodeCreator.constant(math.abs(argument.value));
    return new NodeStatus(newNode, true, MathChangeTypes.ABSOLUTE_VALUE);
  }
  else if (NodeType.isConstantFraction(argument)) {
    const newNumerator = NodeCreator.constant(math.abs(argument.args[0].value));
    const newDenominator =  NodeCreator.constant(math.abs(argument.args[1].value));
    const newNode = NodeCreator.operator('/', [newNumerator, newDenominator]);
    return new NodeStatus(newNode, true, MathChangeTypes.ABSOLUTE_VALUE);
  }
  else {
    throw Error('Absolute value should only have constant arguments')
  }
}
...
function removeDivisionByOne(node) {
  ...
  if (parseFloat(denominator.value) === -1) {
    ...
    return new NodeStatus(numerator, true, MathChangeTypes.DIVISION_BY_NEG_ONE);
  }
  else if (parseFloat(denominator.value) === 1) {
    return new NodeStatus(numerator, true, MathChangeTypes.DIVISION_BY_ONE);
  }
  else {
    return new NodeStatus(node);
  }
