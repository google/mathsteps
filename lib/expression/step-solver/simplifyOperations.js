...
function simplifyOperationsDFS(node) {
  ...
  if (NodeType.isParenthesis(node)) {
    ...
    for (let i = 0; i < node.args.length; i++) {
      ...
      node.args[i] = innerNodeStatus.node;
      if (innerNodeStatus.hasChanged) {
        return new NodeStatus(
          node, innerNodeStatus.changeType, null, innerNodeStatus, i);
      }
    }
  }
  ...
  if (innerNodeStatus.hasChanged) {
    return new NodeStatus(
      node, innerNodeStatus.changeType, null, innerNodeStatus);
  }
  else {
...
function simplifyOperationsBeforeRecursion(node) {
  ...
  for (let i = 0; i < simplificationFunctions.length; i++) {
    ...
    node = removeUnnecessaryParens(nodeStatus.node);
  }
...
function performArithmetic(node) {
  ...
  if (NodeType.isIntegerFraction(node)) {
    ...
    if (numeratorValue % denominatorValue === 0) {
      ...
      return new NodeStatus(newNode, MathChangeTypes.SIMPLIFY_ARITHMETIC);
    }
    else {
      return new NodeStatus(node);
    }
...
function absoluteValue(node) {
  ...
  if (NodeType.isConstant(argument, true)) {
    const newNode = NodeCreator.constant(math.abs(argument.eval()));
    return new NodeStatus(newNode, MathChangeTypes.ABSOLUTE_VALUE);
  }
  else if (NodeType.isConstantFraction(argument, true)) {
    const newNumerator = NodeCreator.constant(math.abs(argument.args[0].eval()));
    const newDenominator =  NodeCreator.constant(math.abs(argument.args[1].eval()));
...
function removeAdditionOfZero(node) {
  ...
  if (zeroIndex >= 0) {
    ...
    return new NodeStatus(node, MathChangeTypes.SIMPLIFY_ARITHMETIC);
  }
  return new NodeStatus(node);
}
...
function reduceExponentByZero(node) {
  ...
  if (NodeType.isConstant(exponent) && exponent.value === '0') {
    ...
    return new NodeStatus(simplifiedNode, changeType);
  }
  else {
    return new NodeStatus(node);
  }
...
function removeMultiplicationByNegativeOne(node) {
  ...
  // Get rid of the -1
  nodeToCombine = Negative.negate(nodeToCombine);
...
function removeDivisionByOne(node) {
  ...
  if (parseFloat(denominator.value) === -1) {
    ...
    return new NodeStatus(numerator, MathChangeTypes.DIVISION_BY_NEG_ONE);
  }
  else if (parseFloat(denominator.value) === 1) {
    return new NodeStatus(numerator, MathChangeTypes.DIVISION_BY_ONE);
  }
  else {
    return new NodeStatus(node);
  }
