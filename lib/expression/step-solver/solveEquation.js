...
const flattenOperands = require('./flattenOperands.js');
...
function addSimplificationSteps(steps, equation, firstTime, debug=false) {
  ...
  for (let i = 0; i < leftSteps.length; i++) {
    ...
    equationStatus = new EquationStatus(equation, step.explanation)
    steps = addStep(steps, equationStatus, debug);
  }
...
function addStep(steps, equationStatus, debug) {
  ...
  let asciimath = equationStatus.equation.print();
  let latex = equationStatus.equation.print(true);
  steps.push({
