  ...
  if (bValue && cValue) {
    ...
    for (const pair of factorPairs) {
      if (pair[0] + pair[1] === bValue) {
        ...
        // SUBSTEP: ax^2 + px + qx + c -> (ax^2 + px) + (qx + c)
        ...
        // SUBSTEP: (ax^2 + px) + (qx + c) -> ux(rx + s) + (qx + c)
        ...
        // SUBSTEP: ux(rx + s) + (qx + c) -> ux(rx + s) + v(rx + s)
        ...
        // SUBSTEP: ux(rx + s) + v(rx + s) -> (ux + v)(rx + s)
        status = Node.Status.nodeChanged(
