...
// Will factor the node if it's in the form of ax^2 + bx + c
function factorQuadraticFormula(node, symbol, aValue, bValue, cValue, negate){
    if (!aValue || !bValue || !cValue){
        return Node.Status.noChange(node);
    }

    const discriminant = math.sqrt(math.pow(b,2) - 4 * a * c);
    const factor1 = (-bValue + discriminant) / (2 * aValue);
    const factor2 = (-bValue - discriminant) / (2 * aValue);
