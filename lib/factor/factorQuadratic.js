  ...
  if (bValue && cValue) {
    ...
    for (const pair of factorPairs) {
      if (pair[0] + pair[1] === bValue) {
        // To factor, we go through some transformations (these should be actual
        // substeps in the future)
        ...
        //    e.g first group: [ux(rx + s)] and second group [v(rx + x)]
        ...
        const u = Node.Creator.constant(math.gcd(aValue, p));
        const r = Node.Creator.constant(aValue/u);
        const s = Node.Creator.constant(p/u);

        // create the first parenthesis
