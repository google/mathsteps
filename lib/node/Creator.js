...
const NodeCreator = {
  ...
  termWithCoefficient (base, exponent, coeff, explicitCoeff=false) {
    let termWithCoeff = base;
    if (exponent) {
      termWithCoeff = this.operator('^', [termWithCoeff, exponent]);
    }
    if (coeff && (explicitCoeff || parseFloat(coeff.value) !== 1)) {
      if (NodeType.isConstant(coeff) &&
        ...
        termWithCoeff = this.unaryMinus(termWithCoeff);
      }
      else {
        termWithCoeff = this.operator('*', [coeff, termWithCoeff], true);
      }
    }
    return termWithCoeff;
  },

  polynomialTerm (symbol, exponent, coeff, explicitCoeff=false) {
    return this.termWithCoefficient(symbol, exponent, coeff, explicitCoeff);
  },
