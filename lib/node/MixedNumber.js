...
function isMixedNumber(node) {
  ...
  const numeratorFirst = NodeType.isUnaryMinus(numerator.args[0]) ?
        Negative.negate(numerator.args[0].args[0])
        : numerator.args[0];

  const numeratorSecond = NodeType.isParenthesis(numerator.args[1]) ?
        numerator.args[1].content
        : numerator.args[1];

  if (!(NodeType.isConstant(numeratorFirst) &&
        NodeType.isConstant(numeratorSecond))) {
    return false;
  }
