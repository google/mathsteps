...
Term.parseNode = function(node, baseNodeFunc, onlyImplicitMultiplication) {
  ...
  // it's '*' ie it has a coefficient
    ...
    if (!NodeType.isConstantOrConstantFraction(coeffNode, true)) {
      throw Error('Expected coefficient to be constant or fraction of ' +
        'constants term, got ' + JSON.stringify(coeffNode));
    }
    coeff = coeffNode;
    const nonCoefficientTerm = new Term(
      node.args[1], baseNodeFunc, onlyImplicitMultiplication);
    if (nonCoefficientTerm.hasCoeff()) {
      throw Error('Cannot have two coefficients ' + JSON.stringify(coeffNode) +
        ' and ' + JSON.stringify(nonCoefficientTerm.getCoeffNode()));
    }
  ...
  // this means there's a fraction coefficient
    ...
    if (!NodeType.isConstant(denominatorNode, true)) {
      throw Error('denominator must be constant node, instead of ' +
        JSON.stringify(denominatorNode));
    }
    ...
    return Term.parseNode(node.body, baseNodeFunc, onlyImplicitMultiplication);
  }
  else {
