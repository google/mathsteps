...
export class Term {
   ...
   * */
  static isTerm(node, baseNodeFunc, onlyImplicitMultiplication = false) {
    try {
  ...
  static parseNode(node, baseNodeFunc, onlyImplicitMultiplication) {
    ...
    if (NodeType.isOperator(node)) {
      ...
      // it's '*' ie it has a coefficient
        ...
        if (!NodeType.isConstantOrConstantFraction(coeffNode)) {
          throw Error(
            "Expected coefficient to be constant or fraction of " +
              "constants term, got " +
              coeffNode
          );
        }
      ...
      // this means there's a fraction coefficient
        ...
        if (!NodeType.isConstant(denominatorNode)) {
          throw Error(
            "denominator must be constant node, instead of " + denominatorNode
          );
        }
        ...
        throw Error(
          "Unsupported operatation for term with coefficent: " + node.op
        );
      }
      ...
      const termNode = new Term(arg, baseNodeFunc, onlyImplicitMultiplication);
