...
NodeType.isFraction = function(node, allowUnaryMinus=true, allowParens=true) {
  if (NodeType.isOperator(node, '/')) {
    return true;
  }
  else if (allowUnaryMinus && NodeType.isUnaryMinus(node)) {
    return NodeType.isFraction(node.args[0]);
  }
  else if (allowParens && NodeType.isParenthesis(node)) {
    return NodeType.isFraction(node.content);
  }

  return false;
};

NodeType.getFraction = function(node, allowUnaryMinus=true, allowParens=true) {
  if (NodeType.isOperator(node, '/')) {
    return node;
  }
  else if (allowUnaryMinus && NodeType.isUnaryMinus(node)) {
    return NodeType.getFraction(node.args[0]);
  }
  else if (allowParens && NodeType.isParenthesis(node)) {
    return NodeType.getFraction(node.content);
  }

  throw Error('Expected fraction');
};

NodeType.hasSymbolInDenominator = function(node, allowUnaryMinus=true, allowParens=true) {
  if (!(NodeType.isFraction(node, allowUnaryMinus=allowUnaryMinus, allowParens=allowParens))) {
    return false;
  }

  const fraction = NodeType.getFraction(node, allowUnaryMinus=allowUnaryMinus, allowParens=allowParens);
  const numerator = fraction.args[0];
  const denominator = fraction.args[1];

  if (NodeType.isConstant(numerator, allowUnaryMinus=allowUnaryMinus) &&
