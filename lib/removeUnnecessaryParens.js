...
function removeUnnecessaryParensDFS(node) {
  if (NodeType.isOperator(node)) {
    ...
    node.args[0] = removeUnnecessaryParensDFS(content);
    return node;
  }
  else {
...
function removeUnnecessaryParensInOperatorNode(node) {
  node.args.forEach((child, i) => {
    node.args[i] = removeUnnecessaryParensDFS(child);
  });
...
function removeUnnecessaryParensInParenthesisNode(node) {
  ...
  if (PolynomialTermNode.isPolynomialTerm(node.content)) {
    ...
    if (node.content.args) {
      node.content.args.forEach((arg, i) => {
        node.content.args[i] = removeUnnecessaryParensDFS(arg);
      });
    }
    node = node.content;
  }
  ...
  // If the content is just one function call, the parens are not needed.
    ...
    node = node.content;
    node = removeUnnecessaryParensDFS(node);
  }
  ...
  // likely needed. So, recurse.
    ...
    node.content = removeUnnecessaryParensDFS(node.content);
  ...
  // to get rid of the extra parens.
    ...
    node = removeUnnecessaryParensDFS(node.content);
  }
