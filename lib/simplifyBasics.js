...
function reduceMultiplicationByZero(node) {
  ...
  if (zeroIndex >= 0) {
    ...
    return NodeStatus.nodeChanged(
      MathChangeTypes.MULTIPLY_BY_ZERO, oldNode, newNode);
  }
  else {
    return NodeStatus.noChange(node);
  }
...
function reduceZeroDividedByAnything(node) {
  ...
  if (node.args[0].value === '0') {
    ...
    const newNode = NodeCreator.constant(0);
    return NodeStatus.nodeChanged(
      MathChangeTypes.REDUCE_ZERO_NUMERATOR, oldNode, newNode);
  }
  else {
    return NodeStatus.noChange(node);
  }
...
function removeAdditionOfZero(node) {
  ...
  if (zeroIndex >= 0) {
    ...
    return NodeStatus.nodeChanged(
      MathChangeTypes.REMOVE_ADDING_ZERO, oldNode, newNode);
  }
  return NodeStatus.noChange(node);
}
...
function removeMultiplicationByOne(node) {
  ...
  if (oneIndex >= 0) {
    ...
    return NodeStatus.nodeChanged(
      MathChangeTypes.REMOVE_MULTIPLYING_BY_ONE, oldNode, newNode);
  }
  return NodeStatus.noChange(node);
}
...
function removeMultiplicationByNegativeOne(node) {
  ...
  return NodeStatus.nodeChanged(
    MathChangeTypes.REMOVE_MULTIPLYING_BY_NEGATIVE_ONE, oldNode, newNode);
}
...
function removeDivisionByOne(node) {
  ...
  if (parseFloat(denominator.value) === -1) {
    ...
    return NodeStatus.nodeChanged(changeType, oldNode, numerator);
  }
