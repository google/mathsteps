...
const isIntegerFraction = (node) => query.isNeg(node) ? isIntegerFraction(node.args[0]) : query.isDiv(node) && node.args.every(arg => Number.isInteger(query.getValue(arg)));
...
function arithmetic(node) {
  ...
  if (!node.args.every(child => query.isNumber(child))) {
    return Node.Status.noChange(node);
  }
  ...
  node.args.forEach((arg, i) => {
    node.args[i] = build.number(evaluate(arg));
  });
  ...
  if (isIntegerFraction(node)) {
    ...
    const newNode = build.number(evaluatedValue);
