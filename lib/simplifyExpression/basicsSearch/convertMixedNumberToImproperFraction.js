...
function convertMixedNumberToImproperFraction(node) {
  ...
  // e.g. 1 2/3
  const wholeNumber = mixedNumber.getWholeNumber(node); // 1
  const numerator = mixedNumber.getNumerator(node); // 2
  const denominator = mixedNumber.getDenominator(node); // 3

  // STEP 1: Convert to unsimplified improper fraction
  // e.g. 1 2/3 -> ((1 * 3) + 2) / 3
  let status = convertToUnsimplifiedImproperFraction(
    newNode, wholeNumber, numerator, denominator);
...
function convertToUnsimplifiedImproperFraction(
  node, wholeNumber, numerator, denominator) {
  let newNode = clone(node);
  ...
  return Node.Status.nodeChanged(
    ChangeTypes.IMPROPER_FRACTION_NUMERATOR, node, newNode);
}
