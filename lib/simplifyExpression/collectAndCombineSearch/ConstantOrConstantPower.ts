...
export function getBaseNode(node) {
  if (node.args) {
    return node.args[0];
  } else {
    return node;
...
export function getExponentNode(node) {
...
export function isConstantOrConstantPower(node) {
  return (
    (NodeType.isOperator(node, "^") && NodeType.isConstant(node.args[0])) ||
