...
function getTermsForCollectingMultiplication(node) {
  ...
  for (let i = 0; i < node.args.length; i++) {
    ...
    else if (child.fn && child.fn.name === 'nthRoot') {
...
// e.g. nthRoot(x, 2), append '2': nthRootNode to terms dictionary
...
function addToTermsforNthRootMultiplication(terms, node) {
  const nthRootNode = clone(node);
  const rootNode = nthRootNode.args[1].value;

  terms = Util.appendToArrayInObject(terms, rootNode, node);
