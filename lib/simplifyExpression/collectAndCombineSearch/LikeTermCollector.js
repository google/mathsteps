...
function getTermName(node, termWithCoefficientSubclass, op) {
  const termWithCoeff = new termWithCoefficientSubclass(node);
  // we 'name' terms by their base node name
  let termName = print(termWithCoeff.getBaseNode());
  // when adding terms, the exponent matters too (e.g. 2x^2 + 5x^3 can't be combined)
  if (op === '+') {
    const exponent = print(termWithCoeff.getExponentNode(true));
    termName += '^' + exponent;
  }
  return termName;
}
