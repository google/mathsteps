...
LikeTermCollector.collectLikeTerms = node => {
    ...
    termTypesSorted.forEach(termType => {
        ...
        // Any like terms should be wrapped in parens.
            ...
            const termList = clone(mathNode.Creator.parenthesis(
                mathNode.Creator.operator(op, termsOfType)));
...
function getTermsForCollectingMultiplication(node: any);
...
// terms list.
function addToTermsforPolynomialMultiplication(terms: any, node: any);
