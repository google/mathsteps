...
function addLikePolynomialTerms(node) {
  ...
  const baseNodeFunc = function(node) {
    return Node.Type.isSymbol(node);
  }

  const changeType = ChangeTypes.ADD_POLYNOMIAL_TERMS;

  return addLikeTermsWithCoefficients(node, baseNodeFunc, changeType);
}

function addLikeNthRootTerms(node) {
  if (!checks.canAddLikeTermWithCoefficientNodes(
...
function addLikeTermsWithCoefficients(node, baseNodeFunc, changeType) {
  const substeps = [];
  ...
  let status = addPositiveOneCoefficient(newNode, baseNodeFunc);
  if (status.hasChanged()) {
  ...
  status = addNegativeOneCoefficient(newNode, baseNodeFunc);
  if (status.hasChanged()) {
    substeps.push(status);
    newNode = Node.Status.resetChangeGroups(status.newNode);
  }
  ...
  status = groupCoefficientsForAdding(newNode, baseNodeFunc);
  substeps.push(status);
  newNode = Node.Status.resetChangeGroups(status.newNode);

  // STEP 4: evaluate the sum (could include fractions)
  status = evaluateCoefficientSum(newNode, baseNodeFunc);
  substeps.push(status);
  newNode = Node.Status.resetChangeGroups(status.newNode);
...
function addPositiveOneCoefficient(node, baseNodeFunc) {
  const newNode = clone(node, false);
  let change = false;

  let changeGroup = 1;
  newNode.args.forEach((child, i) => {
    const termWithCoeff = new Node.TermWithCoefficient(child, baseNodeFunc);
...
// e.g. 2x - x -> 2x - 1x
// Returns a Node.Status object.
function addNegativeOneCoefficient(node, baseNodeFunc) {
...
// Returns a Node.Status object.
function groupCoefficientsForAdding(node, baseNodeFunc) {
  let newNode = clone(node);

  const termList = newNode.args.map(n => new Node.TermWithCoefficient(n, baseNodeFunc));
