...
function addLikePolynomialTerms(node) {
  ...
  return addLikeTermsWithCoefficients(
...
function addLikeNthRootTerms(node) {
  if (!checks.canAddLikeTermNthRootNodes(node)) {
    return Node.Status.noChange(node);
  }
...
// belonging to a subclass of TermWithCoefficient.
function addLikeTermsWithCoefficients(node, termWithCoefficientSubclass, changeType) {
  const substeps = [];
  ...
  let status = addPositiveOneCoefficient(newNode, termWithCoefficientSubclass);
  if (status.hasChanged()) {
  ...
  status = addNegativeOneCoefficient(newNode, termWithCoefficientSubclass);
  if (status.hasChanged()) {
    substeps.push(status);
    newNode = Node.Status.resetChangeGroups(status.newNode);
  }
  ...
  status = groupCoefficientsForAdding(newNode, termWithCoefficientSubclass);
  substeps.push(status);
  newNode = Node.Status.resetChangeGroups(status.newNode);

  // STEP 4: evaluate the sum (could include fractions)
  status = evaluateCoefficientSum(newNode, termWithCoefficientSubclass);
  substeps.push(status);
  newNode = Node.Status.resetChangeGroups(status.newNode);
...
function addPositiveOneCoefficient(node, termWithCoefficientSubclass) {
  const newNode = clone(node, false);
  let change = false;

  let changeGroup = 1;
  newNode.args.forEach((child, i) => {
    const termWithCoeff = new termWithCoefficientSubclass(child);
    if (termWithCoeff.getCoeffValue() === 1) {
      newNode.args[i] = Node.Creator.termWithCoefficient(
        termWithCoeff.getBaseNode(),
        termWithCoeff.getExponentNode(),
...
// e.g. 2x - x -> 2x - 1x
// Returns a Node.Status object.
function addNegativeOneCoefficient(node, termWithCoefficientSubclass) {
...
// Returns a Node.Status object.
function groupCoefficientsForAdding(node, termWithCoefficientSubclass) {
  let newNode = clone(node);

  const termList = newNode.args.map(n => new termWithCoefficientSubclass(n));
  const coefficientList = termList.map(p => p.getCoeffNode(true));
  ...
  // coefficients, but changegroups with termWithCoeff gets messy so let's tackle
  // that later.
  ...
  newNode = Node.Creator.termWithCoefficient(
