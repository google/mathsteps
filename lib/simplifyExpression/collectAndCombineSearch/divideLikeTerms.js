...
function addOneExponent(node) {
  ...
  if (checks.canDivideLikeTermConstantNodes.canDivideLikeTermConstantNodes(node)) {
    newNode.args.forEach((child, i) => {
      if (!checks.canDivideLikeTermConstantNodes.getExponentNode(child)) {
        newNode.args[i] = Node.Creator.powerTerm(
          checks.canDivideLikeTermConstantNodes.getBaseNode(child),
...
function collectConstantExponents(node) {
  ...
  const baseNode = checks.canDivideLikeTermConstantNodes.getBaseNode(node.args[0]);
  // The new exponent will be a difference of exponents (an operation, wrapped in
  // parens) e.g. 10^(5-3)
  const exponentNodeList = node.args.map(p => checks.canDivideLikeTermConstantNodes.getExponentNode(p));
