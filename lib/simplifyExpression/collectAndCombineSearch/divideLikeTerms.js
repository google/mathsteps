...
// Divides a list of nodes that are polynomial like terms or constants with same base. Returns a node.
...
function divideLikeTerms(node, polynomialOnly = false) {
  ...
  let status;
  if (!polynomialOnly && !checks.canDivideLikeTermConstantNodes.canDivideLikeTermConstantNodes(node)) {
...
function dividePolynomialTerms(node) {
  if (!checks.canDivideLikeTermPolynomialNodes(node) && !checks.canDivideLikeTermConstantNodes.canDivideLikeTermConstantNodes(node)) {
    return Node.Status.noChange(node);
  }
  const substeps = [];
...
function addOneExponent(node) {
  ...
  let change = false;
  let changeGroup = 1;
  if (checks.canDivideLikeTermConstantNodes.canDivideLikeTermConstantNodes(node)) {
    newNode.args.forEach((child, i) => {
      const constTerm = new checks.canDivideLikeTermConstantNodes.createPowerNode(child);
      if (!constTerm.exponent) {
        newNode.args[i] = Node.Creator.constantTerm(
          constTerm.base,
...
function collectExponents(node) {
  if (checks.canDivideLikeTermConstantNodes.canDivideLikeTermConstantNodes(node)) {
    const constantTermList = node.args.map(n => new checks.canDivideLikeTermConstantNodes.createPowerNode(n));
    // If we're multiplying constant nodes together, they all share the same
    // base. Get that from the first node.
    const baseNode = constantTermList[0].base;
    // The new exponent will be a sum of negative exponents (an operation, wrapped in
    // parens) e.g. 10^(3+-4+-5)
    const exponentNodeList = constantTermList.map(p => p.exponent);
