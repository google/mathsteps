...
function addOneExponent(node) {
  ...
  if (checks.canDivideLikeTermConstantNodes(node)) {
    newNode.args.forEach((child, i) => {
      const constTerm = new Node.ConstantTerms(child);
      if (!constTerm.getExponentNode()) {
        newNode.args[i] = Node.Creator.constantTerm(
          constTerm.getBaseNode(),
          Node.Creator.constant(1),
          constTerm.getCoeffNode());
...
function collectExponents(node) {
  if (checks.canDivideLikeTermConstantNodes(node)) {
    ...
    const baseNode = constantTermList[0].getBaseNode();
    // The new exponent will be a sum of negative exponents (an operation, wrapped in
    // parens) e.g. 10^(3+-4+-5)
    const exponentNodeList = constantTermList.map(p => p.getExponentNode(true));
    const newExponent = Node.Creator.parenthesis(
      Node.Creator.operator('-', exponentNodeList));
    const newNode = Node.Creator.constantTerm(baseNode, newExponent, null);
    return Node.Status.nodeChanged(
