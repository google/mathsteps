...
// Multiplies a list of nodes that are polynomial or constant power like terms. Returns a node.
...
function multiplyLikeTerms(node, polynomialOnly=false) {
  ...
  if (!polynomialOnly && !checks.canMultiplyLikeTermConstantNodes.canMultiplyLikeTermConstantNodes(node)) {
...
function multiplyPolynomialTerms(node) {
  if (!checks.canMultiplyLikeTermPolynomialNodes(node) && !checks.canMultiplyLikeTermConstantNodes.canMultiplyLikeTermConstantNodes(node)) {
    return Node.Status.noChange(node);
  }
  const substeps = [];
  ...
  if (checks.canMultiplyLikeTermConstantNodes.canMultiplyLikeTermConstantNodes(node)) {
...
// Given a product of polynomial or constant terms, changes any term with no exponent
...
function addOneExponent(node) {
  ...
  if (checks.canMultiplyLikeTermConstantNodes.canMultiplyLikeTermConstantNodes(node)) {
    newNode.args.forEach((child, i) => {
      if (!checks.canMultiplyLikeTermConstantNodes.getExponentNode(child)) {
        newNode.args[i] = Node.Creator.powerTerm(
          checks.canMultiplyLikeTermConstantNodes.getBaseNode(child),
...
function collectConstantExponents(node) {
  ...
  const baseNode = checks.canMultiplyLikeTermConstantNodes.getBaseNode(node.args[0]);
  // The new exponent will be a sum of exponents (an operation, wrapped in
  // parens) e.g. 10^(3+4+5)
  const exponentNodeList = node.args.map(p => checks.canMultiplyLikeTermConstantNodes.getExponentNode(p));
