...
function addOneExponent(node) {
  ...
  if (checks.canMultiplyLikeTermConstantNodes.canMultiplyLikeTermConstantNodes(node)) {
    newNode.args.forEach((child, i) => {
      const constPowerTerm = new checks.canMultiplyLikeTermConstantNodes.createPowerNode(child);
      if (!constPowerTerm.exponent) {
...
function collectConstantExponents(node) {
  const constantTermList = node.args.map(n => new checks.canMultiplyLikeTermConstantNodes.createPowerNode(n));
