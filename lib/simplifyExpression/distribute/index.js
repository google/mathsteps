...
const collectAndCombineSearch = require('../collectAndCombine');
const arithmeticSearch = require('../arithmetic');
const rearrangeCoefficient = require('../basics/rearrangeCoefficient');
...
function distributeAndSimplifyOperationNode(node) {
  ...
  for (let i = 0; i+1 < node.args.length; i++) {
    ...
    if (Node.Type.isOperator(newNode) && newNode.op === '*') {
...
function distributeTwoNodes(firstNode, secondNode) {
  ...
  if (firstArgs.length > 1 && secondArgs.length > 1) {
    firstArgs.forEach(leftArg => {
      let arg = Node.Creator.operator('*', [leftArg, secondNode]);
      arg.changeGroup = 1;
      newArgs.push(arg);
    });
  }
    ...
    firstArgs.forEach(leftArg => {
      secondArgs.forEach(rightArg => {
        let arg = Node.Creator.operator('*', [leftArg, rightArg]);
        arg.changeGroup = 1;
        newArgs.push(arg);
      });
    });
...
function isParenthesisOfAddition(node) {
  ...
  return Node.Type.isOperator(content) && content.op === '+';
}
