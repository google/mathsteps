...
function distribute(node) {
  if (Node.Type.isUnaryMinus(node)) {
    ...
    /*
      ...
      4) If the base is a division node, distribute the exponent
      5) If the base is an addition node, expand the base
    ...
    */
    ...
    if (Negative.isNegative(exponent)
      ...
      return distributeExponentOverMultiplicationOrPower(node);
    }
    else if (Node.Type.isOperator(base, '/')) {
      return distributeExponentOverDivision(node);
    }
    else if (Node.Type.isOperator(base, '+')) {
...
*/
function distributeExponentOverMultiplicationOrPower(node) {
  // Base is required to be a multiplication of terms or a power node
  const base = getContent(node.args[0]);
  ...
  if (Node.Type.isOperator(base, '*')) {
    newTerms = base.args.map(function(term) {
      ...
      if (Node.Type.isOperator(term, '*')) {
        ...
        return distributeExponentOverMultiplicationOrPower(newTerm).newNode;
      }
...
/*
  Given an expression in the form (base)^exponent, where base is a division node
  and the exponent is any term, we want to distribute the exponent over each term.
  Returns a nodeChanged Status object.

  e.g. (x^2/y^2)^(2) -> x^4/y^4
*/
function distributeExponentOverDivision(node) {
  // Base is required to be a division node
  const base = getContent(node.args[0]);
  if (!Node.Type.isOperator(base, '/')) {
    return Node.Status.noChange(node);
  }
  ...
  const numerator = getContent(base.args[0]);
  const denominator = getContent(base.args[1]);

  // Raise the numerator and denominator to the outer exponent
  const newNumerator = Node.Creator.operator('^', [numerator, outerExponent]);
  const newDenominator = Node.Creator.operator('^', [denominator, outerExponent]);
