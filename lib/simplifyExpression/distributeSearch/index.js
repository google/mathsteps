...
// Returns a node.
// Each multiplication node that is created as a result of the distribution
// will have a distribution parameter set equal to `true`. This is useful
// if a client wants to display these nodes differently.
...
function simplify(node) {
  ...
  for (let i = 0; i < newNode.args.length; i++) {
    // Once we try to simplify the distribution term, reset its parameter,
    // since this term is no longer considered the result of distribution.
    // (Even if simplification is not needed, we still reset --
    // e.g. 4(x+y) -> 4x + 4y, we should still reset 4x and 4y)
    delete newNode.args[i].distribution
