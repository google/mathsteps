...
function distribute(node, expressionCtx, parentNode) {
  if (Node.Type.isUnaryMinus(node)) {
    ...
    return expandBase(node, expressionCtx, parentNode)
...
function expandBase (node) {
  ...
  if (!(exponentValue && math.isInteger(exponentValue) && math.larger(exponentValue, 1))) {
    return Node.Status.noChange(node)
  }
  ...
  // We store number constants as bignumber.
  // TODO: Review parseFloat() here.
