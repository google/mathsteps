...
function distributeTwoNodes(firstNode, secondNode) {
  ...
  // If exactly one group contains at least one fraction,
  // AND the denominator of that fraction is equal to the non-fraction group,
  // then we have probably just multiplied both sides by that term.
  // e.g. 3/(x+1) * (x+1)
  // To make it easy to cancel out that term, multiply the non-fraction group
  // into the numerators of the fraction group, rather than performing
  // normal distribution.
  // e.g. 3/(x+1) * (x+1)
  //      instead of (3/(x+1))*x + (3/(x+1))*1
  //      do         3*(x+1)/(x+1)
  // so that we can cancel the like term x+1
  if ([firstArgs, secondArgs].filter(hasFraction).length === 1) {
    ...
    // This boolean indicates whether we have multiplied both sides by a
    // denominator in order to cancel like terms; if so, we should
    // distribute fractions in this special manner
    let cancellingLikeTerms = false;
    fractionNodes.forEach((node) => {
      if (isFraction(node) && node.args[1].equals(nonFractionTerm)) {
        cancellingLikeTerms = true;
      }
    });

    if (cancellingLikeTerms) {
      fractionNodes.forEach((node) => {
        let arg;
        if (isFraction(node)) {
