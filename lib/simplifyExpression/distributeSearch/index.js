...
const nthRootMultiplication = require('../functionsSearch');
...
function distribute(node) {
  if (Node.Type.isUnaryMinus(node)) {
    ...
    return distributeAndSimplifyNthRoots(node);
  }
  else {
...
function distributeAndSimplifyNthRoots (node) {
  // Must be an power node and the exponent must be a constant
  ...
  const newNode = Node.Creator.operator('*', Array(parseFloat(exponent.value)).fill(base));
  console.log(print(newNode));
  const status = nthRootMultiplication(newNode);
  console.log(status)
}
