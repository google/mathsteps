...
function distribute(node) {
  if (Node.Type.isUnaryMinus(node)) {
    ...
    // When the base of the power node is a multiplication node
    // e.g. (x^2 y^2)^2 or ((x+1)(x+2))^2
    if (node.args[0]
        && Node.Type.isParenthesis(node.args[0])
        && Node.Type.isOperator(node.args[0].content, '*')) {
...
function distributeExponent(node) {
  ...
  // TODO: fix for negative exponent
