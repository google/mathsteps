...
function distributeUnaryMinus(node) {
  ...
  const content = unaryContent.body;
  ...
  if (Node.Type.isOperator(content, '*') || Node.Type.isOperator(content, '/')) {
  ...
  else if (Node.Type.isOperator(content, '+')) {
...
function distributeTwoNodes(firstNode, secondNode) {
  ...
  if (isParenthesisOfAddition(secondNode)) {
    secondArgs = secondNode.body.args;
  }
  else {
...
function simplifyWithParens(node) {
  ...
  const status = simplify(node.body);
  if (status.hasChanged()) {
