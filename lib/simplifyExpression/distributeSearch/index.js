...
function distribute(node) {
  if (Node.Type.isUnaryMinus(node)) {
    ...
    /*
      ...
      1) If the exponent is negative, recipricate the base
      2) If the base is a multiplication node, distribute the exponent
      3) Expand the base
    ...
    */
    ...
    if (Negative.isNegative(exponent)
        || (exponent.content && Negative.isNegative(exponent.content))) {
      return recipricateTerm(node);
    }
    else if (base
        && Node.Type.isParenthesis(base)
        && Node.Type.isOperator(base.content, '*')) {
...
/*
  Given a node raised to a negative power, negate the exponent and recipricate
  the node. Returns a nodeChanged Status object.

  e.g. (x y)^-1 -> 1/(x y)
  e.g. (2x^2)^-1 -> 1/(2x^2)
*/
function recipricateTerm(node) {
...
/*
  ...
  e.g. (x^2 y^2)^(2) -> x^4 y^4, (x y)^(1/2) -> nthRoot(x, 2) * nthRoot(y, 2)
...
function distributeExponent(node) {
  ...
  const outerExponent = Node.Type.isParenthesis(node.args[1])
        ? node.args[1].content
        : node.args[1];
  ...
  const newTerms = base.args.map(function(term) {
    ...
    // e.g. innerBase for x is x, 2 is 2, (x + 1) is (x + 1)
    // and (x + 1)^2 is (x + 1)
    const innerBase = Node.Type.isConstant(term) || Node.Type.isSymbol(term) || Node.Type.isParenthesis(term)
          ? term
          : term.args[0];

    // e.g. innerExponent for x is null and (x + 1)^2 is 2
    const innerExponent = Node.Type.isConstant(term) || Node.Type.isSymbol(term) || Node.Type.isParenthesis(term)
          ? null
          : term.args[1];

    // If innerExponent is null, just return the o``uterExponent
    // e.g. (x y)^2 -> x^2 y^2
    // e.g. (x^3 y^3)^2 -> x^(3 * 2) y^(3 * 2)
    const newExponent = innerExponent === null
          ? outerExponent
          : Node.Creator.parenthesis(Node.Creator.operator('*', [innerExponent,outerExponent]));

    return Node.Creator.operator('^', [innerBase, newExponent]);
  });

  const newNode = Node.Creator.operator('*', newTerms);
