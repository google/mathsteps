...
function distribute(node) {
  if (Node.Type.isUnaryMinus(node)) {
    ...
    */
    ...
    else if (base && Node.Type.isParenthesis(base)) {
...
function convertNthRootToExponent(node) {
  ...
  const newExponent = root.equals(exponent)
        ? one
        : Node.Creator.operator('*', [rootExponent, exponent]);
...
function distributeExponent(node) {
  ...
  /*
    1) When the base is a multiplication node
    ...
    2) When the base is sum of terms
    Expand the base
    e.g. (x + 1)^2 -> (x + 1) (x + 1)
  */
