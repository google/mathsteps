...
function expandBase (node) {
  ...
  const expandedNode = Node.Type.isFunction(base, 'nthRoot')
        ? Node.Creator.operator('*', Array(parseFloat(exponent.value)).fill(base))
        : Node.Creator.operator('*', Array(parseFloat(exponent.value)).fill(node.args[0]));
