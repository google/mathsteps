...
function distributeTwoNodes(firstNode, secondNode) {
  ...
  if ([firstArgs, secondArgs].filter(hasFraction).length === 1) {
    const firstArgsFraction = hasFraction(firstArgs);
    const fractionNodes = firstArgsFraction ? firstArgs : secondArgs;
    const nonFractionTerm = firstArgsFraction ? secondNode : firstNode;
    fractionNodes.forEach((node) => {
      let arg;
      if (isFraction(node)) {
        const numerator = Node.Creator.operator('*', [node.args[0], nonFractionTerm]);
...
function isFraction(node) {
  return Node.Type.isOperator(node) && node.op === '/';
}
