...
function cancelLikeTerms(node) {
  ...
  if (Node.Type.isOperator(node, '*')) {
    ...
    const rightNode = newNode.args[1];
    if (Node.Type.isParenthesis(leftNode)) {
      leftNode = leftNode.content;
    }
    // leftNode is a fraction. Check if the denominator is the
    // same as the rightNode being multiplied
    // e.g. 3/(2+x) * (2+x) would simplify to 3
    // leftNode = 3/(2+x)
    // rightNode = (2+x)
    if (Node.Type.isOperator(leftNode, '/')) {
      if (leftNode.args[1].equals(rightNode)) {
        newNode = leftNode.args[0];
        return Node.Status.nodeChanged(
          ChangeTypes.CANCEL_TERMS, node, newNode);
      }
    }
    // leftNode is the sum of multiple terms. Check if all terms
    // have the same denominator and matches the rightNode
    // being multiplied
    // e.g. (3/(2+x) + 5/(2+x)) * (2+x)
    // leftNode = (3/(2+x) + 5/(2+x))
    // rightNode = (2+x)
    if (Node.Type.isOperator(leftNode, '+')) {
      const allMatchingDenominators = leftNode.args.every(function(element) {
        return Node.Type.isOperator(element, '/') && element.args[1].equals(rightNode);
      });
      if (allMatchingDenominators) {
        // Remove the denominators since they all match
        for (let i = 0; i < leftNode.args.length; i++) {
          const currentFraction = leftNode.args[i];
          leftNode.args[i] = currentFraction.args[0];
        }
        newNode = leftNode;
        return Node.Status.nodeChanged(
          ChangeTypes.CANCEL_TERMS, node, newNode);
