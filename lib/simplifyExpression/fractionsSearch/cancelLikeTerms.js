  ...
  if (Node.Type.isOperator(node) && node.op === '*') {
    ...
    if (Node.Type.isOperator(leftNode) && leftNode.op === '+') {
      ...
      for (let i = 0; i < leftNode.args.length; i++) {
        let currentArg = leftNode.args[i];
        if (!Node.Type.isOperator(currentArg) || currentArg.op !== '/') {
          allMatchingDenominators = false;
          break;
        }
        if(!currentArg.args[1].equals(rightNode)) {
          allMatchingDenominators = false;
          break;
        }
      }
      if (allMatchingDenominators) {
        ...
        for (let i = 0; i < leftNode.args.length; i++) {
          let currentFraction = leftNode.args[i];
