...
function cancelLikeTerms(node) {
  ...
  if (!isMultiplicationOfTerms(numerator) &&
    ...
    if (cancelStatus.hasChanged) {
      newNode.args[0] = cancelStatus.numerator || mathNode.Creator.constant(1);
  ...
  // away because we always adjust the exponent in the numerator)
      ...
      !isMultiplicationOfTerms(denominator)) {
    const numeratorArgs = mathNode.Type.isParenthesis(numerator) ?
  ...
  // e.g. x / (x^2*y) => x^(1-2) / y
    ...
    const denominatorArgs = mathNode.Type.isParenthesis(denominator) ?
...
function cancelTerms(numerator, denominator) {
  ...
  if (mathNode.Type.isUnaryMinus(numerator)) {
    ...
    if (!cancelStatus.numerator) {
      numerator = mathNode.Creator.constant(-1);
    }
  ...
  if (mathNode.Type.isUnaryMinus(denominator)) {
  ...
  if (mathNode.PolynomialTerm.isPolynomialTerm(numerator) &&
      mathNode.PolynomialTerm.isPolynomialTerm(denominator)) {
    const numeratorTerm = new mathNode.PolynomialTerm(numerator);
    const denominatorTerm = new mathNode.PolynomialTerm(denominator);
