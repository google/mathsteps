...
function divideByGCD(fraction) {
  ...
  const intermediateNumerator = Node.Creator.operator(
    '/', [Node.Creator.constant(numeratorValue), Node.Creator.constant(gcd)]);
  const intermediateDenominator = Node.Creator.operator(
    '/', [Node.Creator.constant(denominatorValue), Node.Creator.constant(gcd)]);
  const intermediateFraction = Node.Creator.operator(
    '/', [intermediateNumerator, intermediateDenominator]);
  const status = Node.Status.nodeChanged(
    ChangeTypes.FIND_GCD, fraction, intermediateFraction)
  ...
  return Node.Status.nodeChanged(
    ChangeTypes.SIMPLIFY_FRACTION, fraction, newFraction, true, substeps);
}
