...
function divideByGCD(fraction) {
  ...
  let status = Node.Status.nodeChanged(
    ChangeTypes.FIND_GCD, fraction, intermediateFraction, false);
  ...
  status = Node.Status.nodeChanged(
    ChangeTypes.CANCEL_GCD, intermediateFraction, newFraction, false);
  substeps.push(status);
