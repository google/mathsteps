...
function groupTermsByRoot(node) {
  ...
  const terms = radicandNode.args.map(arg => print(arg));

  // Collect groups when there are duplicate terms in the radicand
  if ([...new Set(terms)].length !== terms.length) {
    ...
    radicandNode.args.forEach(arg => {
      arg = print(arg);
      count[arg] ? count[arg]++ : count[arg] = 1;
    });
...
function isMultiplicationOfEqualNodes(node) {
  ...
  const terms = node.args.map(print);

  // return if they are all equal nodes
  return [...new Set(terms)].length === 1;
}
