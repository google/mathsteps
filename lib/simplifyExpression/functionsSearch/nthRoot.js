...
function nthRootExponent(node) {
  ...
  if (print.ascii(rootNode) === print.ascii(exponentNode)) {
    newNode = baseNode;
...
function nthRootMultiplication(node) {
  ...
  if (Node.Type.isConstant(rootNode) && !Negative.isNegative(rootNode)) {
    ...
    if (status.hasChanged()) {
      ...
      // if there's only one exponent group, e.g. nthroot(x^2, 2)
      // we can just evaluate and be done now
...
function groupTermsByRoot(node) {
  ...
  const uniqueTermStrings = Object.keys(count);

  // if there is only one type of term and fewer than the root, then we don't
  // need group them - there's only one group
  // e.g. nthroot(2*2, 2) has only one 2^2 group, so nothing to separate it from
  // e.g. nthroot(2*2*3, 2) has 2^2 and 3 group, so don't return here
  // e.g. nthroot(2*2*2*2*2, 2) has multiply 2^2 groups, so don't return here
  if (uniqueTermStrings.length === 1) {
    const termCount = count[uniqueTermStrings[0]];
    if (termCount <= rootValue) {
      return Node.Status.noChange(node);
    }
  }

  const newTermGroups = uniqueTermStrings.map(key => {
    let leftover = count[key];
    const term = parse(key);
...
function convertMultiplicationToExponent(node) {
  ...
  else if (isMultiplicationOfEqualNodes(radicandNode)) {
    const baseNode = radicandNode.args[0];
    const exponentNode = Node.Creator.constant(radicandNode.args.length);
    const newRadicandNode = Node.Creator.operator('^', [baseNode, exponentNode]);
    newNode.args[0] = newRadicandNode;
    return Node.Status.nodeChanged(
      ChangeTypes.CONVERT_MULTIPLICATION_TO_EXPONENT, node, newNode);
  }
...
function combineRoots(node) {
  ...
  if (children.length > 0) {
    ...
    newNode = Node.Creator.operator('*', children);
    // TODO(math-parser): make an equals function
    if (print.ascii(newNode) === print.ascii(node)) {
      return Node.Status.nodeChanged(
