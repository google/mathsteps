...
const cancelLikeTerms = require('../../simplifyExpression/fractionsSearch/cancelLikeTerms.js');
const removeUnnecessaryParens = require('../../util/removeUnnecessaryParens.js');
const PolynomialTerm = require('../../node/PolynomialTerm.js');
...
function multiplyFractions(node) {
  ...
  // If term is in the form of <constant fraction> <polynomial term>
  // do not multiply in order to keep pedagogical style
  // e.g 1/2 * x^2 -> 1/2 x^2 and not 1x^2 / 2
  const isLinearFraction = Node.PolynomialTerm.isPolynomialTerm(node.args[1]);
  const isConstantFraction = Node.Type.isConstantFraction(node.args[0]);

  if (!atLeastOneFraction || (isLinearFraction && isConstantFraction) ) {
    return Node.Status.noChange(node);
  }
  const numeratorArgs = [];
  ...
  // Apply cancellation 
  const simplifiedNode = cancelLikeTerms(newNode)['newNode'];

  return Node.Status.nodeChanged(
    ChangeTypes.MULTIPLY_FRACTIONS, node, simplifiedNode);
}
