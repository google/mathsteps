...
function multiplyFractions(node) {
  ...
  const atLeastOneFraction = node.args.some(
    arg => Node.Helper.isFraction(arg));
  ...
  node.args.forEach(operand => {
    if (Node.Helper.isFraction(operand)) {
      const fraction = Node.Helper.getFraction(operand);
...
function hasPolynomialInDenominator(node) {
  if (!(Node.Helper.isFraction(node))) {
    return false;
  }

  const fraction = Node.Helper.getFraction(node);
