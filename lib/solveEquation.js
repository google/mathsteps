...
function solveEquation(leftNode, rightNode, comparator, debug=false) {
  ...
  do {
    ...
    try {
      ...
      // This error happens when sometimes for math we don't support
      if (e.message.startsWith('No term with symbol: ')) {
        // eslint-disable-next-line
        console.error(e.message + ', returning no steps');
        return [];
      }
      else {
        throw e; // bubble up
    ...
    if (iters++ === MAX_STEP_COUNT) {
      // eslint-disable-next-line
      console.error('Potential infinite loop for equation: ' +
...
function addSimplificationSteps(steps, equation, debug=false) {
  ...
  let leftSteps = stepThrough(equation.leftNode, false);
  let leftSubSteps = [];
  ...
  if (leftSubSteps.length === 1) {
    ...
    steps = addStep(steps, step, debug);
  }
    ...
    const simplifyStatus = new EquationStatus(
      MathChangeTypes.SIMPLIFY_LEFT_SIDE,
  ...
  let rightSteps = stepThrough(equation.rightNode, false);
  let rightSubSteps = [];
  ...
  if (rightSubSteps.length === 1) {
    ...
    // no change groups are set here - too much is changing for it to be useful
    const simplifyStatus = new EquationStatus(
     MathChangeTypes.SIMPLIFY_RIGHT_SIDE,
...
function addStep(steps, equationStatus, debug) {
  ...
  let substeps = [];
  equationStatus.substeps.forEach(substep => {
    substeps = addStep(substeps, substep, debug);
  });
  steps.push({
    'changeType': equationStatus.changeType,
    'asciimath': equationStatus.newEquation.print(),
    'oldEquation': equationStatus.oldEquation,
    'newEquation': equationStatus.newEquation,
    'substeps': substeps,
  });
  return steps;
}
...
function solveConstantEquation(equation, debug, steps=[]) {
  ...
  let equationStatus = new EquationStatus(changeType, null, equation);
  steps = addStep(steps, equationStatus, debug);
  return steps;
}
