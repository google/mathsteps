...
function addSimplificationSteps(steps, equation, firstTime, debug=false) {
  ...
  if (leftSubSteps.length === 1) {
    ...
    const simplifyStatus = new EquationStatus(                                  // no change marked (too general for change groups to be useful)
...
function solveConstantEquation(equation, debug, steps=[]) {
  ...
  let equationStatus = new EquationStatus(equation, changeType);                // no change group because nothing changed
  steps = addStep(steps, equationStatus, debug);
  return steps;
}
