...
EquationOperations.resolvesToConstantOrParameter = function (node, targetSymbolName) {
  let rv = false

  if (Node.Type.isConstant(node)) {
    // Node is a constant itself.
    rv = true

  } else if (Node.Type.isSymbol(node)) {
    // We treat all symbols other than target one, are treated
    // as parameters.
    rv = (node.name !== targetSymbolName)

  } else if (Node.Type.isParenthesis(node)) {
    // (...) - go on into paranthesis.
    rv = EquationOperations.resolvesToConstantOrParameter(node.content, targetSymbolName)

  } else {
    // Complex node. Process children recursively.
    rv = true
    for (let idx in node.args) {
      if (!EquationOperations.resolvesToConstantOrParameter(node.args[idx], targetSymbolName)) {
        // At least non-constant children found.
        // It's not a constant.
        // Don't go on anymore.
        rv = false
      }
...
EquationOperations.tryRootBothSides = function (equation, symbolName) {
  ...
  if ((left.op === '^') &&
      ...
      EquationOperations.resolvesToConstantOrParameter(right, symbolName)) {
