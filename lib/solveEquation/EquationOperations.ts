...
EquationOperations.ensureSymbolInLeftNode = (equation, symbolName) => {
...
EquationOperations.removeSymbolFromDenominator = equation => EquationStatus.noChange(equation);
...
EquationOperations.removeSymbolFromRightSide = (equation, symbolName) => {
...
// TODO: support inverting functions e.g. sqrt, ^, log etc.
EquationOperations.isolateSymbolOnLeftSide = (equation, symbolName) => {
...
function performTermOperationOnEquation(equation: any, op: any, term: any, changeType: any);
...
function performTermOperationOnExpression(expression: any, op: any, term: any);
function performTermOperationOnExpression(expression, op, term) {
  const node = (mathNode.Type.isOperator(expression) ?
    mathNode.Creator.parenthesis(expression) : expression);

  term.changeGroup = 1;
  const newNode = mathNode.Creator.operator(op, [node, term]);

  return newNode;
}

export = EquationOperations;
