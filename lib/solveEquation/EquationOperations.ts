...
export class EquationOperations {
  ...
  static ensureSymbolInLeftNode(equation, symbolName) {
  ...
  static removeSymbolFromDenominator(equation, symbolName) {
    ...
    const denominator = Symbols.getLastDenominatorWithSymbolTerm(
      leftNode,
      symbolName
    );
    if (denominator) {
  ...
  static removeSymbolFromRightSide(equation, symbolName) {
    ...
    if (PolynomialTerm.isPolynomialTerm(rightNode)) {
      ...
      throw Error("Unsupported node type: " + rightNode);
    }
    return performTermOperationOnEquation(
      equation,
      inverseOp,
  ...
  static isolateSymbolOnLeftSide(equation, symbolName) {
    ...
    if (NodeType.isParenthesis(leftNode)) {
    ...
    if (NodeType.isOperator(leftNode)) {
      if (leftNode.op === "+") {
        ...
        if (NodeType.isConstantFraction(nonSymbolTerm)) {
          ...
          inverseTerm = NodeCreator.operator("/", [
            nonSymbolTerm.args[1],
            nonSymbolTerm.args[0],
          ]);
        } else {
          inverseOp = "/";
      ...
      throw Error("Unsupported node type: " + leftNode);
    }
...
function performTermOperationOnExpression(expression, op, term) {
  const node = NodeType.isOperator(expression)
    ? NodeCreator.parenthesis(expression)
    : expression;

  term.changeGroup = 1;
  const newNode = NodeCreator.operator(op, [node, term]);
