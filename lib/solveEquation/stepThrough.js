...
function stepThrough(leftNode, rightNode, comparator, debug=false) {
  ...
  // This is pretty arbitrary. My thought is that often we isolate for y
  // instead of x, so pick the variable latest in the alphabet.
  // It'd be nice to give the user an option for what variable to solve
  // for though, and eventually we should support that.
  const symbolName = [...symbolSet].sort().pop();
