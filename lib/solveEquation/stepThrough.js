...
function stepThrough(leftNode, rightNode, comparator, debug=false) {
  ...
  do {
    ...
    try {
      ...
      // Conditions: left side is a mul node, right side is 0
      ...
      if (equation.leftNode.op === '*'
          && (Node.Type.isConstant(equation.rightNode) && parseFloat(equation.rightNode.value) === 0)) {
        ...
        for (var f in factors) {
          const leftNode = factors[f].content;
