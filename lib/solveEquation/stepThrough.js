...
function stepThrough(leftNode, rightNode, comparator, debug = false) {
  ...
  // E.g. (33 + 89) (x - 99) = 0
  if (checks.canFindRoots(equation, symbolName)) {
    steps.push(getRootsStatus(equation, symbolName))
    return steps
  }

  // Step through the math equation until nothing changes
  do {
    steps = addSimplificationSteps(steps, equation, symbolName, debug)
    ...
    // Fix issue with 'x+y=x+y' equation.
    // Imported from: https://github.com/GabrielMahan/mathsteps/blob/master/lib/solveEquation/stepThrough.js#L81
  ...
  if (Node.Type.isOperator(equation.leftNode, '^') &&
    Node.Type.isSymbol(equation.leftNode.args[0]) &&
    Node.Type.isConstant(equation.leftNode.args[1])) {
    steps.push(getRootsStatus(equation, symbolName))
  }
...
*/
function getRootsStatus(equation, targetSymbolName) {
  const [solutions, symbol] = getSolutionsAndSymbol(equation, targetSymbolName)
...
function getSolutionsAndSymbol (equation, targetSymbolName) {
  ...
  let symbolNode         = null
  let steps              = null
  let factorsWithSymbols = null

  if (Node.Type.isOperator(leftNode, '^') &&
      Node.Type.isSymbol(leftNode.args[0]) &&
      (leftNode.args[0].name === targetSymbolName) &&
      Node.Type.isConstant(leftNode.args[1])) {

    // x^n = ...
    // Fetch equation parameters.
    const x = leftNode.args[0]
    const n = leftNode.args[1]

    symbolNode = x

    if (Node.Type.kemuIsConstantNegative(rightNode)) {
      // Right side is negative.
      // There is no real solution.
      // Do nothing.
      // Possible improvement: Handle complex solutions.

    } else {
      // Right side is non-negative or unknown. Go on.
      // Calculate solution: x = +/- nthRoot(r, n)
      // Possible improvement: Handle simplification steps on roots.
      // Possible improvement: Handle more than one simplify steps.
      const root = simplify(Node.Creator.nthRoot(rightNode, n))

      if (math.equal(math.mod(n.value, 2), 0)) {
        // x^(2n) = r <=> x = [-nthRoot(r, 2n), -nthRoot(r, 2n)]
        solutions.push(
          simplify(Node.Creator.unaryMinus(root)),
          root)
    ...
    if (Node.Type.isOperator(leftNode, '^') &&
        !EquationOperations.resolvesToConstantOrParameter(leftNode, targetSymbolName)) {
      factorsWithSymbols = [leftNode]
    } else {
      factorsWithSymbols = equation.leftNode.args.filter(arg => !EquationOperations.resolvesToConstantOrParameter(arg, targetSymbolName))
    }
    ...
    */
    ...
    for (let f in factorsWithSymbols) {
      ...
      if ((steps.length === 0) &&
          Node.Type.isSymbol(leftNode) &&
          (leftNode.name === targetSymbolName)) {
        ...
        const lastLeft  = lastStep.newEquation.leftNode
        const lastRight = lastStep.newEquation.rightNode

        // Append to a list of roots
        if ((Node.Type.isSymbol(lastLeft)) &&
            (lastLeft.name === targetSymbolName)) {

          symbolNode = lastLeft
          solutions.push(...Array(exponent).fill(lastRight))
        }
      }
    }
  }

  return [solutions, symbolNode]
}
...
function solveConstantEquation(equation, debug, steps = []) {
  ...
  steps = addSimplificationSteps(steps, equation, null, debug)
...
function addSimplificationSteps(steps, equation, targetSymbolName, debug = false) {
  let oldEquation = equation.clone()

  // (...)^n = ...
  // Nth-root both side for n *NOT* divisible by 2.
  // Possible improvement: Fork equation into two onces for 2n.
  const status = EquationOperations.tryRootBothSides(equation, targetSymbolName)
