...
/*
  ...
  a new EquationStatus will x = -2 as the roots
  ...
  */
  for (var f in factorsWithSymbols) {
    ...
    if (steps.length === 0 && Node.Type.isSymbol(leftNode)) {
      // e.g. x = 0
      symbol = leftNode;
      solutions.push(...Array(exponent).fill(rightNode));
    }
    else {
...
function addSimplificationSteps(steps, equation, debug=false) {
  ...
  /*
    ...
    substeps, proceed to collect simplifying substeps. Push all of these steps
    in leftSubsteps.
  ...
  */
  ...
  // We do not need to collect factoring steps on the right side because we always
  // move the variable term to the left
  // e.g. 0 = x^2 + 3x + 2 -> x^2 + 3x + 2 = 0
