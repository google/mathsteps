...
/*
  ...
  // should we return the root x times or just return it once
*/
...
function getSolutionsAndSymbol (equation) {
  ...
  for (var f in factorsWithSymbols) {
    const factor = factorsWithSymbols[f];

    let leftNode;

    if (Node.Type.isOperator(factor, '^')) {
      leftNode = Node.Type.isParenthesis(factor.args[0])
        ? factor.args[0].content
        : factor.args[0];
    }
    else {
