...
function getSolutionsAndSymbol (equation) {
  ...
  if (Node.Type.isOperator(leftNode, '^') &&
    ...
    // Calculate solution: x = +/- nthRoot(c, n)
    ...
    if (Node.Type.isZero(root)) {
      // Avoid emitting -0, +0 solutions.
      solutions.push(root)

    } else if (math.equal(math.mod(n.value, 2), 0)) {
      // x^(2n) = c <=> x = [-nthRoot(r, 2n), -nthRoot(r, 2n)]
      solutions.push(Node.Creator.unaryMinus(root), root)
    } else {
      // x^(n) = c <=> x = nthRoot(r, n)
      solutions.push(root)
    }
