...
function stepThrough(leftNode, rightNode, comparator, debug=false) {
  ...
  do {
    ...
    try {
      // Find roots of a equation
      // Conditions: left side is a multiplication node, right side is 0
      // TODO: move polynomial to left if is on the right
      // TODO: assuming a single symbol in equation
      // TODO: display all solutions
      // TODO: add optional param for Node.Type.isConstant
      // Returns: An equation in the form of symbol = root
      if (equation.leftNode.op === '*'
          && (Node.Type.isConstant(equation.rightNode)
              && parseFloat(equation.rightNode.value) === 0)) {
        // Gets all factors that aren't a constant
        const factors = equation.leftNode.args.filter(arg => !Node.Type.isConstant(arg));
        ...
        for (var f in factors) {
          ...
          if (allSteps.length > 0) {
            const lastStep = stepThrough(leftNode, rightNode, '=').slice(-1)[0];
...
function addSimplificationSteps(steps, equation, debug=false) {
  ...
  const leftFactorSteps = factor(equation.leftNode, false);
  const leftSubSteps = [];
