...
export function isQuadratic(node) {
  ...
  const constantTerms = node.args.filter(NodeType.isConstant);

  // Check that there is one second degree term and at most one first degree
  // term and at most one constant term
  if (
    secondDegreeTerms.length !== 1 ||
    firstDegreeTerms.length > 1 ||
  ...
  if (
    secondDegreeTerms.length +
      firstDegreeTerms.length +
      constantTerms.length !==
    node.args.length
  ) {
    return false;
  }
