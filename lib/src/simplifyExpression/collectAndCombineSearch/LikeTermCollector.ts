...
import { appendToArrayInObject } from "../../util/Util";
import { NthRootTerm } from "../../node/NthRootTerm";
import { getRootNode } from "../functionsSearch/nthRoot";
...
export class LikeTermCollector {
  ...
  static canCollectLikeTerms(node) {
    ...
    return (
      termTypes.length > 1 && filteredTermTypes.some((x) => terms[x].length > 1)
    );
  }

  // Collects like terms for an operation node and returns a Status object.
  static collectLikeTerms(node) {
    if (!this.canCollectLikeTerms(node)) {
    ...
    termTypesSorted.forEach((termType) => {
      ...
      // Any like terms should be wrapped in parens.
        ...
        const termList = NodeCreator.parenthesis(
...
function getTermName(node, termSubclass, op) {
  ...
  let termName = printAscii(term.getBaseNode());
  // when adding terms, the exponent matters too (e.g. 2x^2 + 5x^3 can't be combined)
  if (op === "+") {
    const exponent = printAscii(term.getExponentNode(true));
    termName += "^" + exponent;
  }
  return termName;
}
...
function getTermsForCollectingAddition(node) {
  ...
  for (let i = 0; i < node.args.length; i++) {
    ...
    if (PolynomialTerm.isPolynomialTerm(child)) {
      const termName = getTermName(child, PolynomialTerm, "+");
      terms = appendToArrayInObject(terms, termName, child);
    } else if (NthRootTerm.isNthRootTerm(child)) {
      const termName = getTermName(child, NthRootTerm, "+");
      terms = appendToArrayInObject(terms, termName, child);
    } else if (NodeType.isIntegerFraction(child)) {
      terms = appendToArrayInObject(terms, CONSTANT_FRACTION, child);
    } else if (NodeType.isConstant(child)) {
      terms = appendToArrayInObject(terms, CONSTANT, child);
    } else if (
      NodeType.isOperator(node) ||
      NodeType.isFunction(node) ||
      NodeType.isParenthesis(node) ||
      NodeType.isUnaryMinus(node)
    ) {
      terms = appendToArrayInObject(terms, OTHER, child);
    } else {
  ...
  if (
    ...
    terms[CONSTANT_FRACTION] &&
    terms[CONSTANT_FRACTION].length === 1
  ) {
    const fraction = terms[CONSTANT_FRACTION][0];
    terms = appendToArrayInObject(terms, CONSTANT, fraction);
...
function getTermsForCollectingMultiplication(node) {
  ...
  for (let i = 0; i < node.args.length; i++) {
    ...
    if (NodeType.isUnaryMinus(child)) {
      terms = appendToArrayInObject(terms, CONSTANT, NodeCreator.constant(-1));
      child = child.args[0];
    }
    if (PolynomialTerm.isPolynomialTerm(child)) {
      terms = addToTermsforPolynomialMultiplication(terms, child);
    } else if (NodeType.isFunction(child, "nthRoot")) {
...
function addToTermsforNthRootMultiplication(terms, node) {
  ...
  terms = appendToArrayInObject(terms, NTH_ROOT + rootNodeValue, node);
...
function addToTermsforPolynomialMultiplication(terms, node) {
  ...
  if (!polyNode.hasCoeff()) {
    termName = getTermName(node, PolynomialTerm, "*");
    terms = appendToArrayInObject(terms, termName, node);
  } else {
    ...
    if (polyNode.getExponentNode()) {
      termWithoutCoefficient = NodeCreator.operator("^", [
        termWithoutCoefficient,
        polyNode.getExponentNode(),
      ]);
    }

    terms = appendToArrayInObject(terms, CONSTANT, coefficient);
    termName = getTermName(termWithoutCoefficient, PolynomialTerm, "*");
    terms = appendToArrayInObject(terms, termName, termWithoutCoefficient);
  }
  return terms;
}
