...
import { ConstantFactors } from "../../factor/ConstantFactors";
...
function nthRootExponent(node) {
  ...
  if (rootNode.equals(exponentNode)) {
    ...
    NodeType.isConstant(rootNode) &&
    NodeType.isConstant(exponentNode)
  ) {
    ...
    if (rootValue % exponentValue === 0) {
      ...
      return NodeStatus.nodeChanged(ChangeTypes.CANCEL_EXPONENT, node, newNode);
      ...
      return NodeStatus.nodeChanged(ChangeTypes.CANCEL_ROOT, node, newNode);
    }
  }
...
function factorMultiplicands(node) {
  ...
  radicandNode.args.forEach((child) => {
    if (PolynomialTerm.isPolynomialTerm(child)) {
      ...
      const polyTermNoCoeff = NodeCreator.polynomialTerm(
...
function getFactorNodes(node) {
  if (NodeType.isConstant(node) && !Negative.isNegative(node)) {
...
function groupTermsByRoot(node) {
  ...
  const termStrings = radicandNode.args.map((arg) => printAscii(arg));
  ...
  radicandNode.args.forEach((arg) => {
    const argString = printAscii(arg);
  ...
  const newTermGroups = termString.map((key) => {
    ...
    while (leftover - times > 0) {
      leftover -= times;
      args.push(
        NodeCreator.parenthesis(
          NodeCreator.operator("*", Array(times).fill(term))
        )
      );
    }
    ...
    const remainder =
      leftover === 1
        ? term
        : NodeCreator.parenthesis(
            NodeCreator.operator("*", Array(leftover).fill(term))
          );
    args.push(remainder);
  ...
  const newBase = NodeCreator.operator("*", newTerms);

  newNode = NodeCreator.nthRoot(newBase, rootNode);

  return NodeStatus.nodeChanged(ChangeTypes.GROUP_TERMS_BY_ROOT, node, newNode);
}
...
function convertMultiplicationToExponent(node) {
  ...
  } else if (NodeType.isOperator(radicandNode, "*")) {
    const children = [];
    radicandNode.args.forEach((child) => {
...
function distributeNthRoot(node) {
  ...
  return NodeStatus.nodeChanged(ChangeTypes.DISTRIBUTE_NTH_ROOT, node, newNode);
}
...
function combineRoots(node) {
  ...
  if (children.length > 0) {
    if (radicandArgs.length > 0) {
      const radicandNode =
        radicandArgs.length === 1
          ? radicandArgs[0]
          : NodeCreator.operator("*", radicandArgs);
...
function nthRootConstant(node) {
  ...
  if (math.pow(roundedNthRootValue, rootValue) === radicandValue) {
    newNode = NodeCreator.constant(roundedNthRootValue);
    return NodeStatus.nodeChanged(ChangeTypes.NTH_ROOT_VALUE, node, newNode);
  }
  ...
  // pulled out of the radical
    ...
    if (factors.length > 1) {
      ...
      substeps.push(
        NodeStatus.nodeChanged(ChangeTypes.FACTOR_INTO_PRIMES, node, newNode)
      );
...
export function getRadicandNode(node) {
  if (!NodeType.isFunction(node, "nthRoot")) {
    throw Error("Expected nthRoot");
  }
...
function isMultiplicationOfEqualNodes(node) {
  ...
  const termStrings = node.args.map(printAscii);
