...
import { EquationStatus } from "../equation/Status";
...
import { canFindRoots } from "../checks/canFindRoots";
import { NodeCreator } from "../node/Creator";
import { NodeType } from "../node/NodeType";
import { resolvesToConstant } from "../checks/resolvesToConstant";
import { EquationOperations } from "./EquationOperations";

import { stepThrough as simplifyExpressionNode } from "../simplifyExpression/stepThrough";
import { stepThrough as factor } from "../factor/stepThrough";
...
export function stepThrough(leftNode, rightNode, comparator, debug = false) {
  ...
  if (
    hasUnsupportedNodes(equation.leftNode) ||
      ...
      if (equationStatus.newEquation.ascii().length > 300) {
        ...
        throw Error(
          "Math error: Potential infinite loop for equation " +
            originalEquationStr +
            ". It reached over 300 characters " +
...
function getRootsStatus(equation) {
  ...
  if (solutions.length > 1) {
    ...
    allSolutions = NodeCreator.list(flattenSolutionsList);
  } else if (solutions.length === 1) {
    allSolutions = solutions[0];
  } else {
    allSolutions = NodeCreator.list([]);
  }
...
function getSolutionsAndSymbol(equation) {
  ...
  if (NodeType.isOperator(leftNode, "^") && !resolvesToConstant(leftNode)) {
    factorsWithSymbols = [leftNode];
  } else {
    factorsWithSymbols = equation.leftNode.args.filter(
      (arg) => !resolvesToConstant(arg)
    );
  }
  ...
  */
  ...
  for (var f in factorsWithSymbols) {
    ...
    if (NodeType.isOperator(factor, "^")) {
      exponent = parseFloat(factor.args[1].value);
      factor = factor.args[0];
    }

    const leftNode = NodeType.isParenthesis(factor) ? factor.content : factor;

    steps = stepThrough(leftNode, equation.rightNode, "=");

    if (steps.length === 0 && NodeType.isSymbol(leftNode)) {
      // e.g. x = 0
      symbol = leftNode;
      ...
      if (NodeType.isSymbol(lastStep.newEquation.leftNode)) {
...
function solveConstantEquation(equation, debug, steps = []) {
  ...
  steps = addSimplificationSteps(steps, equation, true);
  if (steps.length > 0) {
    const lastStep = steps[steps.length - 1];
    equation = Equation.createEquationFromString(
      lastStep.newEquation.ascii(),
  ...
  if (
    !NodeType.isConstantOrConstantFraction(equation.leftNode, true) ||
    !NodeType.isConstantOrConstantFraction(equation.rightNode, true)
  ) {
    throw Error(
      "Expected both nodes to be constants, instead got: " + equation.ascii()
    );
  }
...
function addSimplificationSteps(steps, equation, debug = false) {
  ...
  */
  ...
  const simplifiedLeftNode =
    leftSimplifySteps.length !== 0
  ...
  if (leftSubSteps.length === 1) {
    ...
    const finalEquation = EquationStatus.resetChangeGroups(
      lastStep.newEquation
    );
