...
// Returns a node.
export function removeUnnecessaryParens(node, rootNode = false) {
...
function removeUnnecessaryParensSearch(node: MathNode): MathNode {
...
function removeUnnecessaryParensInOperatorNode(node) {
  ...
  if (node.op === "^" && NodeType.isParenthesis(node.args[0])) {
    const base = node.args[0];
    if (NodeType.isOperator(base.content)) {
  ...
  // e.g. `(2 + x) - (1 + x)` => `2 + x - (1 + x)` not `2 + x - 1 + x`
    ...
    if (
      NodeType.isParenthesis(node.args[0]) &&
...
function removeUnnecessaryParensInParenthesisNode(node) {
  ...
  // needed.
    ...
    NodeType.isConstant(node.content, true) ||
