...
THIS COMMENT NEEDS UPDATING - we should still flatten just to be sure, but
...
function flattenOperands(node) {
  ...
  // This is mostly for convenience. The evaluate() changes unary minuses
  // around constant nodes to constant nodes with negative values.
  // Then we can use one node instead of two to represent the same thing.
  if (Node.Type.isConstant(node, true)) {
    ...
    if ('+-*'.includes(mathParserOpToSymbol(node.op))) {
      let parentOp;
      if (Node.Type.isOperator(node, '-')) {
        ...
        parentOp = mathParserOpToSymbol(node.op);
      }
    ...
    node.body = flattenOperands(node.body);
    return node;
  }
...
function flattenSupportedOperation(node, parentOp) {
  ...
  if (operands.length === 1) {
    ...
    if (Node.Type.isOperator(node, '/') &&
        (operands.length > 2 || hasMultiplicationBesideDivision(node))) {
...
function getOperands(node, parentOp) {
  ...
  const nodeOp = mathParserOpToSymbol(node.op);
  if (!'+-*/'.includes(nodeOp)) {
    return [flattenOperands(node)];
  }

  // subtraction is flattened under addition, so it's operation would be +
  if (nodeOp === '-') {
    if (parentOp !== '+') {
      return [flattenOperands(node)];
    }
  }
  else if (parentOp !== nodeOp) {
    return [flattenOperands(node)];
  }
  ...
  else if (parentOp === '*' && mathParserOpToSymbol(node.op) === '/') {
...
function mathParserOpToSymbol(op) {
  switch (op) {
  case 'mul':
    return '*';
  case 'div':
