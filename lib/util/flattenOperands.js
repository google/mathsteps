...
const isVariableFactor = (node) => {
  ...
  if (node.type === 'OperatorNode' && node.op === '^' && isVariableFactor(node.args[0])) {
    return true;
  }
  return false;
};
...
const isPolynomialTerm = (node) => {
  if (node.type === 'OperatorNode' && node.op === '*') {
    // TODO(kevinb) handle fraction coefficients
    if (node.args[0].type === 'ConstantNode') {
      for (let i = 1; i < node.args.length; i++) {
        if (!isVariableFactor(node.args[i])) {
          return false;
        }
      }
      return true;
...
const {replaceMathJS} = require('../../parser/docs/bundle');

function newFlattenOperands(node) {
  const path = [];

  const newAST = replaceMathJS(node, {
    enter(node) {
    ...
    leave(node) {
      if (node.type === 'OperatorNode' && node.op === '+') {
        ...
        return Node.Creator.operator('+', args, node.implicit);
      } else if (node.type === 'OperatorNode' && node.op === '*') {
        const args = [];

        for (const arg of node.args) {
          if (arg.type === 'OperatorNode' && arg.op === '*' && !arg.implicit) {
            args.push(...arg.args);
          } else if (arg.type === 'OperatorNode' && arg.op === '/' && arg.args[0].type === 'SymbolNode' && args.length > 0 && args[args.length - 1].type === 'ConstantNode') {
            const coeff = args.pop();
            const symbol = arg.args[0];
            const denominator = arg.args[1];
            args.push(
              Node.Creator.operator(
                '/',
                [
                  Node.Creator.operator('*', [coeff, symbol], true),
                  denominator,
                ]
              )
            );
        ...
        if (args.length > 1) {
          return Node.Creator.operator('*', args, isPolynomialTerm(node));
        } else {
          return args[0];
        }
      } else if (node.type === 'OperatorNode' && node.fn === 'unaryMinus') {
        // TODO(kevinb) update tests/searchers to deal with unaryMinus without
        // having to convert it to a negative constant.  This will allow us to
        // differentiate subtraction from adding a negative number
        const child = node.args[0];
        if (child.type === 'ConstantNode' && parseFloat(child.value) > 0) {
          return Node.Creator.constant(`-${child.value}`);
        } else if (child.type === 'OperatorNode' && '*/'.indexOf(child.op) !== -1) {
          const [first, ...rest] = child.args;
          if (first.type === 'ConstantNode' && parseFloat(first.value) > 0 && rest.every((node) => node.type === 'ConstantNode')) {
            return Node.Creator.operator(child.op, [
              Node.Creator.constant(`-${first.value}`),
              ...rest,
            ]);
          }
        }
...
module.exports = newFlattenOperands;
