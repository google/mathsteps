  ...
  return replace(root, {
    leave(node) {
      if (query.isAdd(node)) {
        ...
        // e.g. 2 * x^2 * y -> 2 x^2 y
        if (node.args.every(query.isPolynomialTerm) && query.isNumber(node.args[0])
        ...
        // the numerator.
        // e.g. 2 x / 3 -> 2x / 3
        // TODO (kevinb): only rewrites first two factors of multiplication
        else if (canRearrangeFraction(node)) {
          return build.div(
            build.implicitMul(node.args[0], node.args[1].args[0]),
            node.args[1].args[1]);
        }
        // Flatten multiplication nest mul nodes as long as they're all implicit
        // or explicit.
        // e.g. (2 * 3) * 4 -> 2 * 3 * 4
        // e.g. (2 x) (y z) -> 2 x y z
        else {
...
function canRearrangeFraction(node) {
  if (!node.args) return false;
  if (!query.isDiv(node.args[1])) return false;
  const fraction = node.args[1];
  return query.isNumber(node.args[0])
    && query.isIdentifier(query.getNumerator(fraction))
    && node.implicit;
}
