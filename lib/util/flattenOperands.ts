...
export function flattenOperands(node) {
  ...
  if (NodeType.isConstant(node, true)) {
    ...
    const constNode = NodeCreator.constant(evaluate(node));
    if (node.changeGroup) {
  ...
  } else if (NodeType.isFunction(node) && node.fn.args) {
    ...
    const newNode = NodeCreator.operator("*", [
      flattenedFn,
      NodeCreator.parenthesis(flattenedArg),
    ]);
    return newNode;
...
function getOperands(node, parentOp) {
  ...
  if (PolynomialTerm.isPolynomialTerm(node, true)) {
...
function isPolynomialTermMultiplication(node) {
  ...
  // with no coefficient)
  const secondOperand = node.args[1];
  if (PolynomialTerm.isPolynomialTerm(secondOperand)) {
    const polyNode = new PolynomialTerm(secondOperand);
...
function maybeFlattenPolynomialTerm(node) {
  ...
  if (NodeType.isConstantOrConstantFraction(lastOperand)) {
    ...
    operands.push(NodeCreator.operator("*", [lastOperand, nextOperand], true));
  }
