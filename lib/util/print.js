...
function printTreeTraversal(node, parentNode) {
  ...
  if (Node.Type.isOperator(node)) {
    ...
    if (needToAddParens(node, parentNode)) {
      str = `(${str})`;
    }
...
// next to [/, *, ^] operation or parenthesis node
// Examples:
// { "/" [{ "+" ["x", "2"] }, "2"] } -> (x + 2) / 2.
// { "+" [{ "+" ["x", "x"] }, { "+" ["2", "2"] }] } -> (x + x) + (2 + 2)
// Check #120, #126 issues for more details.
function needToAddParens(node, parentNode) {
  if (!parentNode ||
  ...
  if ('+-'.indexOf(parentNode.op) === -1 ||
      parentNode.args.some(Node.Type.isParenthesis)) {
    return true;
  }
