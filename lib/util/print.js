...
function printLatex(node, showPlusMinus=false) {
  ...
  function intoTex(node, showPlusMinus=false) {
    ...
    const nodeSymbols = Symbols.getSymbolsInExpression(node);

    // Creates the regex parts separately so that current symbol can be added
    // in the middle
    const regexFirstHalf = '\\s*?\\\\cdot\\s*?({?)\\s*?';
    const regexSecondHalf = '\\s*?(}?)';

    // Checks each symbol for explicit multiplication and removes it
    for (const symbol of nodeSymbols) {
      const regex = new RegExp(regexFirstHalf + '('+ symbol + ')' +
        regexSecondHalf, 'g');

      outputNodeTex = outputNodeTex.replace(regex,
        '$1$2$3');
    }

    // Removes whitespace
    outputNodeTex = outputNodeTex.replace(/\s/g, '');

    return outputNodeTex;
  }

  return intoTex(node, showPlusMinus);
}

module.exports = print;
module.exports.latex = printLatex;
