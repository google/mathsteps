  ...
  if (Node.Type.isOperator(node)) {
    ...
    switch (node.op) {
      ...
      case '^': {
        ...
        if (!Node.Type.isParenthesis(exponentNode) &&
            !Node.Type.kemuIsConstantOrSymbol(exponentNode)) {
          // Wrap complex exponent into parenthesis.
          // Example: x^(3/4)
          return printTreeTraversal(baseNode) + '^(' + printTreeTraversal(exponentNode) + ')'
        }
