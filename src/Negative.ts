...
static isNegative(node: mNode): boolean {
  if (nodeHelper.Type.isUnaryMinus(node)) {
    return !Negative.isNegative(node.args[0]);
  }
  else if (nodeHelper.Type.isConstant(node)) {
    return parseFloat(node.value) < 0;
  }
  else if (nodeHelper.Type.isConstantFraction(node)) {
  ...
  else if (nodeHelper.PolynomialTerm.isPolynomialTerm(node)) {
    const polyNode = new nodeHelper.PolynomialTerm(node);
...
//    not naive: -3 -> 3, x -> -x
//    naive: -3 -> --3, x -> -x
static negate(node: mNode, naive=false): mNode {
  if (nodeHelper.Type.isConstantFraction(node)) {
    ...
    if (nodeHelper.Type.isUnaryMinus(node)) {
      ...
      return nodeHelper.Creator.constant(0 - parseFloat(node.value));
    }
  }
  return nodeHelper.Creator.unaryMinus(node);
};
...
//    not naive: -3x -> 3x, x -> -x
//    naive: -3x -> --3x, x -> -x
static negatePolynomialTerm(node: mNode, naive=false): mNode {
  ...
  if (!polyNode.hasCoeff()) {
    newCoeff = nodeHelper.Creator.constant(-1);
  }
  else {
    ...
    if (oldCoeff.value === '-1') {
      ...
      newCoeff = nodeHelper.Creator.operator('/', [numerator, denominator]);
    }
    else {
  ...
  return nodeHelper.Creator.polynomialTerm(
