...
function flattenOperands(node: mNode): mNode {
  if (nodeHelper.Type.isConstant(node, true)) {
    // the evaluate() changes unary minuses around constant nodes to constant nodes
    // with negative values.
    const constNode = nodeHelper.Creator.constant(evaluate(node));
    if (node.changeGroup) {
  ...
  else if (nodeHelper.Type.isFunction(node, 'abs')) {
    node.args[0] = flattenOperands(node.args[0]);
    return node;
  }
  else if (nodeHelper.Type.isFunction(node, 'nthRoot')) {
    node.args[0] = flattenOperands(node.args[0]);
    if (node.args[1]) {
...
function flattenSupportedOperation(node: mNode, parentOp) {
  ...
  if (operands.length === 1) {
    ...
    if (node.op === '/' && (operands.length > 2 ||
      ...
      node = nodeHelper.Creator.operator('*', operands);
    }
...
function getOperands(node: mNode, parentOp) {
  ...
  if (nodeHelper.PolynomialTerm.isPolynomialTerm(node, true)) {
...
function isPolynomialTermMultiplication(node: mNode) {
  ...
  // with no coefficient)
  const secondOperand = node.args[1];
  if (nodeHelper.PolynomialTerm.isPolynomialTerm(secondOperand)) {
    const polyNode = new nodeHelper.PolynomialTerm(secondOperand);
...
// multiplied together.
function maybeFlattenPolynomialTerm(node: mNode) {
  ...
  if (nodeHelper.Type.isConstantOrConstantFraction(lastOperand)) {
    // we replace the constant (which we popped) with constant*symbol
    operands.push(
      nodeHelper.Creator.operator('*', [lastOperand, nextOperand], true));
  }
...
function flattenDivision(node: mNode) {
  // We recurse on the left side of the tree to find operands so far
  ...
  if (operands.length === 1) {
    ...
    const divisionNode = nodeHelper.Creator.operator('/', [numerator, denominator]);
...
function hasMultiplicationBesideDivision(node: mNode) {
  if (!nodeHelper.Type.isOperator(node)) {
    return false;
  }
  if (node.op === '*') {
