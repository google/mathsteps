...
import { NodeMixedNumber } from "../../lib/src/node/MixedNumber";

function testIsMixedNumber(input, output) {
  TestUtil.testBooleanFunction(NodeMixedNumber.isMixedNumber, input, output);
}

function testIsNegativeMixedNumber(input, output) {
  TestUtil.testBooleanFunction(
    NodeMixedNumber.isNegativeMixedNumber,
    input,
    output
  );
}
...
function testGetWholeNumberValue(input, output) {
  ...
  TestUtil.testFunctionOutput(
    NodeMixedNumber.getWholeNumberValue,
    input,
    output
  );
}
...
function testGetNumeratorValue(input, output) {
  input = TestUtil.parseAndFlatten(input);
  TestUtil.testFunctionOutput(NodeMixedNumber.getNumeratorValue, input, output);
}

function testGetDenominatorValue(input, output) {
  input = TestUtil.parseAndFlatten(input);
  TestUtil.testFunctionOutput(
    NodeMixedNumber.getDenominatorValue,
    input,
    output
  );
}
