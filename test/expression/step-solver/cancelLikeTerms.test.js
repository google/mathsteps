...
function testCancelLikeTerms(exprStr, expectedStr) {
  const evaluted = flatten(cancelLikeTerms(flatten(math.parse(exprStr))).node);
  assert.deepEqual(
    evaluted,
    flatten(math.parse(expectedStr)));
  return
}
...
describe('cancel like terms', function () {
  const tests = [
    ...
    ['(x^3*y)/x^2', '(x^(3-2)*y)'],
    ['-(7+x)^8/(7+x)^2', '-(7+x)^(8-2)'],
    ['(2x^2 * 5) / (2x^2)', '5'], // these parens have to stay around 2x^2 to be parsed correctly. TODO: make sure I'm not removing them in remove uncessary parens
    ['(x^2 * y) / x', '(x^(2 - 1) * y)'],
    ['2x^2 / (2x^2 * 5)', '1 / 5'],
    ['x / (x^2*y)', 'x^(1-2) / y'],
    ['(4x^2) / (5x^2)', '(4)/(5)'],
