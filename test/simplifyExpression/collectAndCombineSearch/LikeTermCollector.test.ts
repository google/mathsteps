...
function testCollectLikeTerms(
  ...
  it(description, () => {
    ...
    const collected = printAscii(
      LikeTermCollector.collectLikeTerms(exprTree).newNode
    );
    if (debug) {
...
function testCanCollectLikeTerms(exprStr, canCollect, explanation) {
  ...
  it(description, () => {
    const exprTree = TestUtil.parseAndFlatten(exprStr);
    assert.equal(LikeTermCollector.canCollectLikeTerms(exprTree), canCollect);
  });
}
...
describe("basic addition collect like terms, no exponents or coefficients", function () {
  const tests = [
    ...
    [
      "y + nthRoot(x, 2) + 4y + nthRoot(x, 2)",
      "(nthRoot(x, 2) + nthRoot(x, 2)) + (y + 4y)",
    ],
    [
      "nthRoot(x, 2) + 2 + nthRoot(x, 2) + 5",
      "(nthRoot(x, 2) + nthRoot(x, 2)) + (2 + 5)",
    ],
  ];
...
describe("collect like terms with exponents and coefficients", function () {
  const tests = [
    ...
    [
      "4x + 7 * nthRoot(11) - x - 2 * nthRoot(11)",
      "(7 * nthRoot(11) - 2 * nthRoot(11)) + (4x - x)",
    ],
  ];
  tests.forEach((t) => testCollectLikeTerms(t[0], t[1], t[2]));
});
...
describe("collect like terms for nthRoot multiplication", function () {
  const tests = [
    ...
    [
      "nthRoot(x, 2) * nthRoot(x, 2) * 3",
      "3 * (nthRoot(x, 2) * nthRoot(x, 2))",
    ],
    [
