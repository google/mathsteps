...
import LikeTermCollector = require('../../../lib/simplifyExpression/collectAndCombineSearch/LikeTermCollector');
...
function testCanCollectLikeTerms(exprStr: any, canCollect: any, explanation: any);
...
describe('can collect like terms for addition', () => {
    const tests = [
...
describe('can collect like terms for multiplication', () => {
    const tests = [
        ['2*2', false, 'because only one type'],
        ['x^2 * 2x^2', true],
        ['x * 2', false, 'because all types have only one'],
...
describe('collect like terms with exponents and coefficients', () => {
    const tests = [
...
describe('collect like terms for multiplication', () => {
    const tests = [
